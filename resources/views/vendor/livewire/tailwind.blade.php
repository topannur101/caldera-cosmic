            ...
            <div class="flex justify-between flex-1 sm:hidden">
                <span>
                    @if ($paginator->onFirstPage())
                        <button type="button" disabled class="px-4 py-2 font-semibold text-sm hover:bg-neutral-50 dark:hover:bg-neutral-700  disabled:opacity-25 rounded-md transition ease-in-out duration-150"><i class="icon-chevron-left"></i></button>
                    @else
                        <button type="button" wire:click="previousPage('{{ $paginator->getPageName() }}')" x-on:click="{{ $scrollIntoViewJsSnippet }}" wire:loading.attr="disabled" dusk="previousPage{{ $paginator->getPageName() == 'page' ? '' : '.' . $paginator->getPageName() }}.before" class="px-4 py-2 font-semibold text-sm hover:bg-neutral-50 dark:hover:bg-neutral-700 focus:outline-none focus:ring-2 focus:ring-caldy-500 focus:ring-offset-2 dark:focus:ring-offset-neutral-800 disabled:opacity-25 rounded-md transition ease-in-out duration-150"><i class="icon-chevron-left mr-2"></i>{{__('Sebelumnya')}}</button>
                    @endif
                </span>

                <span>
                    @if ($paginator->hasMorePages())
                        <button type="button" wire:click="nextPage('{{ $paginator->getPageName() }}')" x-on:click="{{ $scrollIntoViewJsSnippet }}" wire:loading.attr="disabled" dusk="nextPage{{ $paginator->getPageName() == 'page' ? '' : '.' . $paginator->getPageName() }}.before" class="px-4 py-2 font-semibold text-sm hover:bg-neutral-50 dark:hover:bg-neutral-700 focus:outline-none focus:ring-2 focus:ring-caldy-500 focus:ring-offset-2 dark:focus:ring-offset-neutral-800 disabled:opacity-25 rounded-md transition ease-in-out duration-150">{{__('Selanjutnya')}}<i class="icon-chevron-right ml-2"></i></button>
                    @else
                        <button type="button" disabled class="px-4 py-2 font-semibold text-sm hover:bg-neutral-50 dark:hover:bg-neutral-700  disabled:opacity-25 rounded-md transition ease-in-out duration-150"><i class="icon-chevron-right"></i></button>
                    @endif
                </span>
            </div>
                ...
                <div class="ml-auto">
                    <span class="relative z-0 inline-flex">
                        <span>
                            ...
                            @else
                                <button type="button" wire:click="previousPage('{{ $paginator->getPageName() }}')" x-on:click="{{ $scrollIntoViewJsSnippet }}" dusk="previousPage{{ $paginator->getPageName() == 'page' ? '' : '.' . $paginator->getPageName() }}.after" rel="prev" class="px-4 py-2 font-semibold text-sm hover:bg-neutral-50 dark:hover:bg-neutral-700 focus:outline-none focus:ring-2 focus:ring-caldy-500 focus:ring-offset-2 dark:focus:ring-offset-neutral-800 disabled:opacity-25 rounded-md transition ease-in-out duration-150" aria-label="{{ __('pagination.previous') }}"><i class="icon-chevron-left"></i>
                                </button>
                            @endif
                        </span>
