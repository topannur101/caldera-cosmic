...
>
    <x-text-button
        ...
        type="button"
        x-on:click.prevent="$dispatch('open-modal', 'search-filter')"
        ::class="
            filter || aging
    ...
    >
        ...
        <span x-text="filter || aging ? filter_name + ' ' + aging_name : '{{ __("Filter") }}'"></span>
    </x-text-button>
    <x-modal name="search-filter" maxWidth="sm">
        <div class="p-6 flex flex-col gap-y-6">
            ...
            <div>
                ...
                <div class="px-5">
                    <x-text-button class="text-xs uppercase font-semibold" type="button" x-on:click="filter = ''; $dispatch('close');" x-show="filter">
                        <span class="text-red-500"><div class="px-1">{{ __("Reset") }}</div></span>
                    </x-text-button>
                </div>
            </div>
            <div>
                ...
                <div class="px-5">
                    <x-text-button class="text-xs uppercase font-semibold" type="button" x-on:click="aging = ''; $dispatch('close');" x-show="aging">
                        <span class="text-red-500"><div class="px-1">{{ __("Reset") }}</div></span>
                    </x-text-button>
                </div>
            </div>
