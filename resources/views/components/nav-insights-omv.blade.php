<header id="cal-nav-omv" class="bg-white dark:bg-neutral-800 shadow">
    <div class="flex justify-between max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="space-x-3 -my-px ml-10 flex">
            <x-nav-link class="text-sm px-3 uppercase" href="{{ route('insight.omv.index') }}" :active="request()->routeIs('insight.omv.index')" wire:navigate>
                <i class="fa fa-pen text-sm"></i><span class="ms-3 hidden md:inline">{{ __('Buat') }}</span>
            </x-nav-link>
            <x-nav-link class="text-sm px-3 uppercase" href="{{ route('insight.omv.summary') }}" :active="request()->routeIs('insight.omv.summary')" wire:navigate>
                <i class="fa fa-heart-pulse text-sm"></i><span class="ms-3 hidden md:inline">{{ __('Data') }}</span>
            </x-nav-link>
            <x-nav-link class="text-sm px-3 uppercase" href="{{ route('insight.omv.manage.index') }}" :active="request()->routeIs('insight.omv.manage.index')" wire:navigate>
