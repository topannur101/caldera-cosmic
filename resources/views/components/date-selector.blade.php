@props(['isQuery' => false ])
<div x-data="{
        date_from: @entangle('date_from').live,
        date_to: @entangle('date_to').live,
        get date_facade() {
            if (!this.date_from.trim() && !this.date_to.trim()) {
                return '';
            }
            return `${this.date_from} — ${this.date_to}`.trim();
        },
        setToday() {
            const today = new Date();
            this.date_from = this.formatDate(today);
            this.date_to = this.formatDate(today);
        },
        setYesterday() {
            const yesterday = new Date();
            yesterday.setDate(yesterday.getDate() - 1);
            this.date_from = this.formatDate(yesterday);
        ...
        setThisWeek() {
            ...
            this.date_from = this.formatDate(startOfWeek);
            this.date_to = this.formatDate(today);
        },
        setLastWeek() {
            const today = new Date();
            ...
            this.date_from = this.formatDate(lastMonday);
        ...
        setThisMonth() {
            ...
            this.date_from = this.formatDate(startOfMonth);
            this.date_to = this.formatDate(today);
        },
        setLastMonth() {
            const today = new Date();
            ...
            this.date_from = this.formatDate(startOfLastMonth);
        ...
        <div class="grid grid-cols-1 gap-y-6 p-6">
            <div class="flex justify-between items-start">
                <h2 class="text-lg font-medium text-neutral-900 dark:text-neutral-100">
                    <i class="fa fa-fw fa-calendar me-3"></i>{{ __('Pilih tanggal') }}
                </h2>
            ...
            <div class="grid grid-cols-2 gap-y-6 gap-x-3">        
                <div>
                    <label for="date_from" class="block px-3 mb-2 uppercase text-xs text-neutral-500">{{ __('Dari') }}</label>
                    <x-text-input id="date_from" type="date" x-model="date_from" x-on:keydown.enter.prevent="$nextTick(() => { $refs.dateTo.focus() })" />
                </div>                          
                <div>
                    <label for="date_to" class="block px-3 mb-2 uppercase text-xs text-neutral-500">{{ __('Sampai') }}</label>
                    <x-text-input id="date_to" type="date" x-model="date_to" x-ref="dateTo" x-on:keydown.enter.prevent="$dispatch('close')" />
                </div>
            </div>  
            <div class="flex flex-wrap gap-2">
                ...
                <x-secondary-button size="sm" type="button" x-on:click="clearDates();$dispatch('close');" x-show="date_to || date_from"><span class="text-red-500"><div class="px-1">{{ __('Hapus filter tanggal') }}</div></span></x-secondary-button>
            </div>
        </div>
    </x-modal>  
</div>