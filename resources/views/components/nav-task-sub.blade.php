   ...
   <div class="flex justify-between max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
       <div>
            <h2 class="flex items-center gap-x-4 font-semibold text-xl text-neutral-800 dark:text-neutral-200 leading-tight">
                ...
                @else          
                    <x-link href="{{ route('tasks.dashboard') }}" class="inline-block px-3 py-6" wire:navigate>
                        <i class="icon-arrow-left"></i>
                    </x-link>
                    <div>
                        <span class="hidden sm:inline">{{ $slot }}</span>
       ...
       <div class="space-x-8 -my-px ml-10 flex">
            <x-nav-link class="text-sm px-1 uppercase" href="{{ route('tasks.dashboard') }}" :active="request()->is('tasks') || request()->is('tasks/dashboard')" wire:navigate>
                <i class="icon-gauge text-sm"></i><span class="ms-3 hidden lg:inline">{{ __('Dashboard') }}</span>
            </x-nav-link>
            ...
            <x-nav-link class="text-sm px-1 uppercase" href="{{ route('tasks.items.index') }}" :active="request()->is('tasks/items*')" wire:navigate>
                <i class="icon-list-todo text-sm"></i><span class="ms-3 hidden lg:inline">{{ __('Tugas') }}</span>
            </x-nav-link>
        </div>
   </div>
</header>