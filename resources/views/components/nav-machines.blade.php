    ...
    <div class="flex justify-between max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="space-x-8 -my-px ml-10 flex">
            <x-nav-link class="text-sm px-1 uppercase" href="{{ route('inventory.items.index') }}" :active="request()->is('inventory/items*')" wire:navigate>
                ...
                <span class="ms-3 hidden lg:inline">{{ __("Direktori") }}</span>
            </x-nav-link>
            <x-nav-link class="text-sm px-1 uppercase" href="{{ route('inventory.circs.index') }}" :active="request()->is('inventory/circs*')" wire:navigate>
                <i class="icon-wrench text-sm"></i>
                <span class="ms-3 hidden lg:inline">{{ __("Perawatan ") }}</span>
            </x-nav-link>
            <x-nav-link class="text-sm px-1 uppercase" href="#" :active="request()->is('inventory/reqs*')" wire:navigate>
                <i class="icon-clock-arrow-down text-sm"></i>
                <span class="ms-3 hidden lg:inline">{{ __("Waktu henti") }}</span>
            </x-nav-link>
        </div>
        {{--
            <div class="space-x-8 -my-px ml-10 flex">
