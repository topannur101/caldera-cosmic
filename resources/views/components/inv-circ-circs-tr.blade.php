        ...
        <td class="w-[1%]">
            <label for="{{ "circ-" . $id }}" class="flex p-2 gap-x-2">
                <input
                    {{ $checked ? "checked" : "" }}
                    {{
                        $attributes->merge(["class" => "w-4 h-4 text-caldy-600 bg-neutral-100 border-neutral-300 rounded focus:ring-2 focus:ring-caldy-500 dark:focus:ring-caldy-600 dark:ring-offset-neutral-800 dark:bg-neutral-700 dark:border-neutral-600"])
                    }}
                    id="{{ "circ-" . $id }}"
                    type="checkbox"
                    value="{{ $id }}"
        ...
        <div class="rounded-sm overflow-hidden relative flex w-12 h-6 bg-neutral-200 dark:bg-neutral-700">
            ...
            @if ($item_photo)
                <img
                    ...
                    src="{{ "/storage/inv-items/" . $item_photo }}"
                />
            @endif
        </div>
    </td>
