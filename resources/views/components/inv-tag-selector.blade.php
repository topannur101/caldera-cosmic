    ...
    x-data="{
        ...
        async updateHints() {
            ...
            try {
                ...
                const response = await fetch(
                    `/api/inv-tags?q=${encodeURIComponent(query)}`,
                )
        ...
        addTag() {
            ...
            if (this.tags.length >= 3) {
                toast('{{ __("Hanya maksimal 3 tag diperbolehkan") }}', {
                    type: 'danger',
                })
                return
            }
...
>
    <x-text-button
        ...
        type="button"
        x-on:click.prevent="$dispatch('open-modal', 'tag-selector')"
        ::class="
            tag_list
        ...
        "
    >
        <i class="icon-tag me-3"></i>
        <span x-text="tag_list ? tag_list : '{{ __("Tag") }}'"></span>
    </x-text-button>
        ...
        <div>
            <div class="p-6 flex flex-col gap-y-6">
                ...
                <div class="flex justify-end">
                    <x-text-button class="text-xs uppercase font-semibold" type="button" x-on:click="tags = []; $dispatch('close');" x-show="tags.length">
