...
>
    <x-text-button
        ...
        type="button"
        x-on:click.prevent="$dispatch('open-modal', 'user-selector')"
        ::class="
            user_name
    ...
    >
        ...
        <span x-show="!user_name" x-text="'{{ $isQuery ? __("Pengguna") : __("Filter pengguna") }}'"></span>
        <span class="max-w-16 truncate" x-show="user_name" x-text="user_name"></span>
    </x-text-button>
    <x-modal name="user-selector">
        <div class="grid grid-cols-1 gap-y-6 p-6">
            ...
            <div class="grid grid-cols-3 gap-2">
                <template x-for="user in users" :key="user.id">
                    <x-secondary-button size="sm" type="button" x-on:click="user_id = user.id; $dispatch('close')">
                        <div class="flex justify-between gap-x-2 truncate">
                            <div>
                                <div class="w-4 h-4 bg-neutral-200 dark:bg-neutral-700 rounded-full overflow-hidden">
                                    <img x-show="user.photo" class="w-full h-full object-cover dark:brightness-75" :src="'/storage/users/' + user.photo" />
                                    <svg
                                        x-show="!user.photo"
            ...
            <div class="flex justify-end">
                <x-text-button class="text-xs uppercase font-semibold" type="button" x-on:click="user_id = 0;$dispatch('close');" x-show="user_id">
                    <span class="text-red-500"><div class="px-1">{{ __("Hapus filter pengguna") }}</div></span>
                </x-text-button>
            </div>
        </div>
    </x-modal>
