...
<nav x-data="{ open: false }" class="fixed inset-x-0 top-0 z-[60] bg-white dark:bg-neutral-800 border-b border-neutral-100 dark:border-neutral-700">
    ...
    <div class="px-4 md:px-6 lg:px-8">
        <div class="flex h-16">
            <div class="flex grow">
                ...
                <div class="px-3 shrink-0 flex items-center">
                    <a href="{{ route("home") }}" wire:navigate>
                        <x-application-logo class="block h-6 w-auto fill-current text-neutral-800 dark:text-neutral-200" />
                    </a>
                ...
                <div id="cal-nav-main-links-alt" class="hidden text-xs uppercase">
                    <span
                        class="inline-flex items-center ms-10 h-full border-b-2 border-transparent font-medium leading-5 text-neutral-500 dark:text-neutral-400 dark:hover:border-neutral-700"
                    >
                ...
                <div class="my-auto text-neutral-700 dark:text-neutral-300">
                    <x-dropdown align="right" width="72">
                        <x-slot name="trigger">
                            <button
                                wire:click="ackNotif"
                                x-on:click="document.getElementById('cal-notif-counter')?.remove()"
            ...
            <div class="hidden md:flex md:items-center md:ms-6">
                ...
                @else
                    <div
                        class="flex items-center gap-x-4 text-xs uppercase font-medium leading-5 text-neutral-500 dark:text-neutral-400 hover:text-neutral-700 dark:hover:text-neutral-300"
                    >
                        <livewire:layout.navigation-lang-set :route="url()->current()" small="true" />
                        <div>
                            <x-link :href="route('login')" wire:navigate>{{ __("Masuk") }}</x-link>
                        </div>
                    </div>
                @endif
            </div>
            ...
            @if ($user["id"])
                ...
                <div class="mt-3 space-y-1">
                    <x-responsive-nav-link :href="route('account')" :active="request()->routeIs('account*')" wire:navigate>
                        <i class="icon-user-pen px-3"></i>
                        <span class="px-3">{{ __("Akun") }}</span>
                    </x-responsive-nav-link>

                    @if ($user["id"] == 1)
                        <x-responsive-nav-link :href="route('admin')" :active="request()->routeIs('admin*')" wire:navigate>
                            <i class="icon-cog px-3"></i>
                            <span class="px-3">{{ __("Administrasi") }}</span>
                        </x-responsive-nav-link>
                    @endif
                    ...
                    <button wire:click="logout" class="w-full text-start">
                        <x-responsive-nav-link>
                            <i class="icon-power px-3"></i>
                            <span class="px-3">{{ __("Keluar") }}</span>
                        </x-responsive-nav-link>
                    </button>
                </div>
            @else
