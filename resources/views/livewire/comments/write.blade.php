    ...
    public function updatedUserq()
    {
        if ($this->userq) {
            ...
            $this->users = User::where(function (Builder $query) use ($q) {
                $query->orWhere("name", "LIKE", "%" . $q . "%")->orWhere("emp_id", "LIKE", "%" . $q . "%");
            })
                ->where("is_active", 1)
                ->take(5)
                ->get();
    ...
    <form wire:submit.prevent="save" class="flex gap-x-4" wire:target="save" wire:loading.class="opacity-30">
        ...
        <div class="w-full">
            <textarea
                ...
                name="comment"
                x-ref="comment"
                x-on:focusin="buttons = true"
                x-on:input="updateUserq"
                placeholder="{{ __("Tulis komentar...") }}"
                style="min-height: 66px"
                class="block w-full border-neutral-300 dark:border-neutral-700 dark:bg-neutral-900 dark:text-neutral-300 focus:border-caldy-500 dark:focus:border-caldy-600 focus:ring-caldy-500 dark:focus:ring-caldy-600 rounded-md shadow-sm"
            ></textarea>
            ...
            <div wire:key="files" class="flex justify-between items-start mt-3">
                @if (count($files))
                    <div class="text-sm">
                        ...
                        <x-text-button type="button" wire:click="resetFiles" class="mt-3">
                ...
                @else
                    <div class="text-sm">
                        <x-text-button type="button" x-on:click="$refs.upload.click()">
