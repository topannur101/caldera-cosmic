...
<div>
    <div @if($isForm) Wire:click.away="apply" @endif class="p-6">
        ...
        <div class="flex mt-3">
            <div class="w-full">
                <x-text-input-icon wire:model.live="tags.{{ $i }}" type="text" icon="icon-tag" id="tag{{ $i }}" list="qtags{{ $i }}"></x-text-input-icon>
            </div>
            <x-text-button wire:click="removeTag({{ $i }})" type="button" class="ms-3"><i class="icon-x"></i></x-text-button>
        </div>
        <datalist id="qtags{{ $i }}">
            @if(isset($qtags[$i]))
        ...
        <div wire:key="addTag">
            @if(count($tags) < 5)
                <x-text-button type="button" class="mt-3" wire:click="addTag"><i class="icon-plus mr-2"></i>{{ __('Tambah tag') }}</x-text-button>
            @endif
        </div>
        @if(!$isForm)
        <div class="flex">
