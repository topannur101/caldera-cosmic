...
new #[Layout("layouts.app")] class extends Component {
    ...
    private function InvCircQuery()
    {
        ...
        $inv_circs_query = InvCirc::with(["inv_stock", "inv_stock.inv_item", "inv_stock.inv_item.inv_area", "inv_curr", "user"])
    ...
    public function with(): array
    {
        ...
        $user_ids = $inv_circs_query
            ->limit(1000)
            ->get()
            ->pluck("user_id")
            ->unique();
        $this->users = User::whereIn("id", $user_ids)
            ->orderBy("name")
            ->get()
            ->toArray();
...
<div
    id="content"
    class="py-6 max-w-8xl mx-auto sm:px-6 lg:px-8 text-neutral-800 dark:text-neutral-200"
    x-data="{
        ...
        getAllIds() {
            return Array.from(
                document.querySelectorAll('input[type=checkbox][id^=circ-]'),
            ...
            <div class="p-6 flex flex-col gap-y-6">
                ...
                <div class="flex justify-end">
                    <x-secondary-button type="button" wire:click="printAs" x-on:click="$dispatch('close')">
                        <div class="relative">
                            <span wire:loading.class="opacity-0" wire:target="printAs">
                                <i class="icon-printer"></i>
                                <span class="ml-0 hidden md:ml-2 md:inline">{{ __("Cetak") }}</span>
                            </span>
            ...
            <div class="flex gap-x-2">
                ...
                <div class="btn-group">
                    <x-secondary-button type="button" wire:click="printCircIds">
                        <div class="relative">
                            <span wire:loading.class="opacity-0" wire:target="printCircIds">
                                <i class="icon-printer"></i>
                                <span class="ml-0 hidden md:ml-2 md:inline">{{ __("Cetak") }}</span>
                            </span>
                    ...
                    <x-secondary-button type="button" wire:click="evalCircIds">
                        <div class="relative">
                            <span wire:loading.class="opacity-0" wire:target="evalCircIds">
                                <i class="icon-gavel"></i>
                                <span class="ml-0 hidden md:ml-2 md:inline">{{ __("Evaluasi") }}</span>
                            </span>
