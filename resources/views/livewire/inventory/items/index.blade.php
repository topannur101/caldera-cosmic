...
new #[Layout("layouts.app")] class extends Component {
    ...
    public function with(): array
    {
        ...
        return [
            ...
            "inv_items_count" => $inv_stocks_query
                ->build()
                ->distinct("inv_item_id")
                ->count("inv_item_id"),
        ];
    }
    ...
    <div wire:key="modals">
            ...
            <div class="p-6 flex flex-col gap-y-6">
                ...
                <div class="flex justify-end">
                    <x-secondary-button type="button" wire:click="download" x-on:click="$dispatch('close')">
                        <div class="relative">
                            <span wire:loading.class="opacity-0" wire:target="download">
                                <i class="icon-download"></i>
                                <span class="ml-0 hidden md:ml-2 md:inline">{{ __("Unduh") }}</span>
                            </span>
        ...
        <x-modal name="hidden-warning" focusable>
            <div class="p-6 flex flex-col gap-y-6">
                ...
                <div class="text-sm">
                    {{ __('Menggunakan penyortiran "Jarang diambil", "Sering diambil" dan "Lokasi" akan menyebabkan barang tanpa frekuensi pengambilan atau tanpa lokasi menjadi tersaring.') }}
                </div>
                <div class="flex justify-end">
                                        ...
                                        <div class="rounded-sm overflow-hidden relative flex w-12 h-4 bg-neutral-200 dark:bg-neutral-700">
                                            ...
                                            @if ($inv_stock->inv_item->photo)
                                                <img
                                                    ...
                                                    src="{{ "/storage/inv-items/" . $inv_stock->inv_item->photo }}"
                                                />
                                            @endif
                                        </div>
                                    </td>
                                    <td class="max-w-40 truncate font-bold">
                                        <x-link href="{{ route('inventory.items.show', [ 'id' => $inv_stock->inv_item_id, 'stock_id' => $inv_stock->id ]) }}" wire:navigate>
                                            {{ $inv_stock->inv_item->name }}
                                        </x-link>
                                    </td>
