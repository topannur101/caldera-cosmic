...
new class extends Component {
    ...
    public function init()
    {
        ...
        $this->can_eval = $inv_item
            ? Gate::inspect("circEval", $inv_item)->allowed()
            : false;
        $this->can_create = $inv_item
            ? Gate::inspect("circCreate", $inv_item)->allowed()
            : false;
        ...
        <div class="px-4 text-sm text-neutral-500 border-b border-neutral-200 dark:border-neutral-700">
            <ul class="flex flex-wrap uppercase">
                @foreach ($stocks as $stock)
                    <li>
                        <div
                            wire:click="$set('stock_id', {{ $stock["id"] }})"
                            class="cursor-pointer inline-block px-6 py-3 border-b-2 @if ($stock["id"] == $stock_id)
                                text-neutral-800
                                dark:text-neutral-200
                                border-caldy-500
                                active
                                dark:border-caldy-500
                            @else
                                border-transparent
