...
new #[Layout("layouts.app")] class extends Component {
    ...
    public function update()
    {
        ...
        const incompleteBasics = " .
                json_encode($this->incompleteBasics()) .
                ";
    ...
    public function agingTable()
    {
        ...
        $tags = InvTag::whereHas("inv_items", function ($query) {
            $query->where("inv_area_id", $this->area_id)->where("is_active", true);
        })
            ->orderBy("name")
            ->get();
        ...
        foreach ($tags as $key => $tag) {
            ...
            $this->progress += ($key + 1) / $tags->count() / 1.34;
            $this->stream(to: "progress", content: min(floor($this->progress / 10), 100), replace: true);
        }
        ...
        foreach ($itemsWithNoTags as $key => $item) {
            ...
            $this->progress += ($key + 1) / $itemsWithNoTags->count() / 1.34;
            $this->stream(to: "progress", content: min(floor($this->progress / 10), 100), replace: true);
        }
    ...
    public function aging()
    {
        ...
        $gt100DaysCount = InvItem::where(function ($q) use ($sub_100_days) {
            $q->where("last_withdrawal", "<", $sub_100_days)->orWhereNull("last_withdrawal");
        })
            ->where("inv_area_id", $this->area_id)
            ->count();
...
<div
    x-data="{
        ...
        aging_tag_highest: @entangle("aging_tag_highest"),
    }"
    x-init="observeProgress()"
        ...
        <div wire:loading.class.remove="hidden" class="hidden">
            <div class="w-72">
                <div class="flex justify-between text-sm">
                    ...
                    <div>
                        <span wire:stream="progress">{{ round($progress / 10, 0) }}</span>
                        %
                    </div>
                </div>
                <div class="cal-shimmer mt-1 relative w-full bg-neutral-200 rounded-full h-1.5 dark:bg-neutral-700">
                    <div class="bg-caldy-600 h-1.5 rounded-full dark:bg-caldy-500 transition-all duration-200" :style="'width:' + progress + '%'" style="width: 0%"></div>
                </div>
            </div>
        </div>
    </div>
    ...
    <div x-cloak :class="area_id ? (agingTable ? '' : 'cal-shimmer') : 'hidden'">
        <div class="grid grid-cols-6 gap-4">
            <div class="col-span-3 bg-white dark:bg-neutral-800 shadow sm:rounded-lg p-4">
                ...
                <div wire:ignore id="incomplete-basics-container" class="h-48 overflow-hidden" wire:key="incomplete-basics-container"></div>
            </div>
            <div class="bg-white dark:bg-neutral-800 shadow sm:rounded-lg p-4">
                ...
                <div wire:ignore id="status-container" class="h-48 overflow-hidden" wire:key="status-container"></div>
            </div>
            <div class="col-span-2 bg-white dark:bg-neutral-800 shadow sm:rounded-lg p-4">
                ...
                <div wire:ignore id="aging-container" class="h-48 overflow-hidden" wire:key="aging-container"></div>
            </div>
        </div>
        <div class="mt-4">
            ...
            return {
                ...
                updateTable(tableData) {
                    ...
                    this.agingTable = new Tabulator('#aging-data-table', {
                        ...
                        columns: [
                            ...
                            },
                            {
                                ...
                                formatter: function (cell) {
                                    ...
                                    let url = tag
                                        ? '{{ url("/inventory/items") }}?tags[0]=' +
                                          encodeURIComponent(tag) +
                                          '&aging=gt-100-days' +
                                          areaParam +
                                          '&ignore_params=true&sort=amt_high&view=list'
                        ...
                        initialSort: [{ column: 'tag_name', dir: 'asc' }],
                    });
                },
            };
        }
