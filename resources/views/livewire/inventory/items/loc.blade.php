<div @if(!$isForm) class="p-6" @endif>
    ...
    <x-text-input-icon x-ref="input" wire:model.live="loc" icon="icon-map-pin" id="loc" list="qlocs" type="text" placeholder="{{ __('Lokasi') }}" />
    <datalist id="qlocs">
        @if (count($qlocs))
            @foreach ($qlocs as $qloc)
                <option wire:key="{{ "qloc" . $loop->index }}" value="{{ $qloc }}"></option>
            @endforeach
        @endif
    </datalist>
    @error("loc")
