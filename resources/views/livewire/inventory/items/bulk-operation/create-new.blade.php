   ...
   public function apply(bool $is_confirmed = false)
   {
      if ($is_confirmed) {
         ...
         foreach ($this->items as $item) {
            $this->result['items'][] = $this->createItem($item);
         }         
         ...
         if($this->result['failure']) {
            ...
            // CSV download
            $filename = __('Hasil operasi massal (buat barang)') . ' ' . Carbon::now()->format('Y-m-d His') . '.csv';
            $handle = fopen('php://temp', 'r+');
            $headers = [
               'name', 'desc', 'code', 'location', 'tag_0', 'tag_1', 'tag_2',
...
<x-slot name="title">{{ __('Buat barang baru') . ' — ' . __('Inventaris') }}</x-slot>
   ...
   @if (count($areas))
            ...
            <div class="p-6 space-y-4 text-sm">
               ...
               @if(count($result['items']))
                  ...
                  <div class="p-6 border border-neutral-200 dark:border-neutral-700 rounded-lg">
                     ...
                     @if($result['success'])
                     <div>
                        <x-pill color="green">{{ $result['success'] }}</x-pill>{{ ' ' . __('barang dibuat.') }}                     
                     </div>
                     @endif
                     @if($result['failure'])
                     <div>
                        <x-pill color="red">{{ $result['failure'] }}</x-pill>{{ ' ' . __('barang gagal dibuat.') }}                     
                     </div>
                     @endif
                  </div>
                  @endif
               ...
               @else
                  <div>
                     <x-pill>{{ $count }}</x-pill>{{ ' ' . __('barang akan dibuat.') }}
                  </div>
                  <div>
            ...
            <div x-data="{ backup: false }" class="p-6 space-y-4 text-sm">
               ...
               <div x-show="!backup" class="p-6 border border-neutral-200 dark:border-neutral-700 rounded-lg">
                  ...
                  <p class="leading-relaxed">
                     {{ __('Tempel daftar barang yang hendak kamu buat di kotak editor. Maksimal 100 entri dalam sekali operasi.') }}
                  </p>
               </div>
      ...
      x-init="editorInit()">
         <div class="flex flex-col sm:flex-row gap-y-6 justify-between px-6 mb-8">
            <h1 class="text-2xl text-neutral-900 dark:text-neutral-100"><i class="fa-star-of-life mr-3"></i>{{ __('Buat barang baru') }}</h1>
...
<script type="module">   
   Alpine.data('editorData', () => ({
         ...
         editorReset() {
            Livewire.navigate("{{ route('inventory.items.bulk-operation.create-new') }}");
         },

         editorDownload() {
            this.table.download("csv", "operasi-massal-buat-baru.csv"); 
         },
   }));
</script>
@endscript