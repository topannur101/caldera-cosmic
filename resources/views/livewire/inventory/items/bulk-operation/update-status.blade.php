...
class extends Component
{
   ...
   public function apply(bool $is_confirmed = false)
   {
      if ($is_confirmed) {
         ...
         if($this->result['failure']) {
            ...
            // CSV download
            $filename = __('Hasil operasi massal (perbarui status)') . ' ' . Carbon::now()->format('Y-m-d His') . '.csv';
         ...
         foreach ($this->items as $key => $item) {
            ...
            foreach ($item as $childKey => $childValue) {
               ...
               if (in_array($childKey, ['code', 'is_active'])) {
   ...
   {      
      try {
         ...
         $inv_item->is_active = $item['is_active'];
         $inv_item->save();
         
         $status  = __('Berhasil');
         $message = __('Status barang diperbarui');
...
<x-slot name="title">{{ __('Perbarui status') . ' — ' . __('Inventaris') }}</x-slot>
   ...
   @if (count($areas))
            ...
            <div class="p-6 space-y-4 text-sm">
               ...
               @if(count($result['items']))
                  ...
                  <div class="p-6 border border-neutral-200 dark:border-neutral-700 rounded-lg">
                     ...
                     @if($result['success'])
                     <div>
                        <x-pill color="green">{{ $result['success'] }}</x-pill>{{ ' ' . __('status barang diperbarui.') }}                     
                     </div>
                     @endif
                     @if($result['failure'])
                     <div>
                        <x-pill color="red">{{ $result['failure'] }}</x-pill>{{ ' ' . __('status barang gagal diperbarui.') }}                     
                     </div>
                     @endif
                  </div>
                  @endif
               ...
               @else
                  <div>
                     <x-pill>{{ $count }}</x-pill>{{ ' ' . __('status barang akan diperbarui.') }}
                  </div>
                  <div>
                     {{ __('Di area mana status barang tersebut akan diperbarui?') }}
                  </div>
                  <div>
                     <label for="area_id"
            ...
            <div x-data="{ backup: false }" class="p-6 space-y-4 text-sm">
               ...
               <div x-show="!backup" class="p-6 border border-neutral-200 dark:border-neutral-700 rounded-lg">
                  ...
                  <p class="leading-relaxed">
                     {{ __('Tempel daftar barang yang hendak kamu perbarui statusnya di kotak editor. Maksimal 100 entri dalam sekali operasi.') }}
                  </p>
               </div>
      ...
      x-init="editorInit()">
         <div class="flex flex-col sm:flex-row gap-y-6 justify-between px-6 mb-8">
            <h1 class="text-2xl text-neutral-900 dark:text-neutral-100"><i class="fa fa-fw fa-toggle-on mr-3"></i>{{ __('Perbarui status') }}</h1>
   ...
   Alpine.data('editorData', () => ({
         ...
         editorInit() {
            const columns = [
               ...
               { title: 'is_active', field: 'is_active', width: 80 },
            ];
         ...
         editorReset() {
            Livewire.navigate("{{ route('inventory.items.bulk-operation.update-status') }}");
         },

         editorDownload() {
            this.table.download("csv", "operasi-massal-perbarui-status.csv"); 
         },
   }));
</script>
@endscript