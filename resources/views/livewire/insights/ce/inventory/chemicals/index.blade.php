...
new #[Layout("layouts.app")] class extends Component {
    ...
    public string $download_as = "inv_ce_stocks";
    ...
    public function mount()
    {
        ...
        if (! $this->ignore_params) {
            ...
            if ($chemicalsParams) {
                $this->q = $chemicalsParams["q"] ?? "";
                $this->name = $chemicalsParams["name"] ?? "";
                $this->desc = $chemicalsParams["desc"] ?? "";
                $this->code = $chemicalsParams["code"] ?? "";
                $this->is_linked = $chemicalsParams["is_linked"] ?? true;
                $this->loc_parent = $chemicalsParams["loc_parent"] ?? "";
                $this->loc_bin = $chemicalsParams["loc_bin"] ?? "";
                $this->uom = $chemicalsParams["uom"] ?? "";
                $this->tags = $chemicalsParams["tags"] ?? [];
                $this->area_ids = $chemicalsParams["area_ids"] ?? [];
                $this->filter = $chemicalsParams["filter"] ?? "";
                $this->aging = $chemicalsParams["aging"] ?? "";
                $this->limit = $chemicalsParams["limit"] ?? "";
                $this->view = $chemicalsParams["view"] ?? "content";
                $this->sort = $chemicalsParams["sort"] ?? "";
            }

            $areasParams = session("inv_ce_areas_params", []);
    ...
    public function download()
    {
        ...
        switch ($this->download_as) {
            case "inv_ce_stocks":
                ...
                return redirect()->route("download.inv-ce-stocks", ["token" => $token]);

            case "inv_ce_chemicals":
                session()->put("inv_ce_chemicals_token", $token);
                return redirect()->route("download.inv-ce-chemicals", ["token" => $token]);
        }
    }
    ...
    public function updated($property)
    {
        ...
        if ($property == "loc_parent") {
            ...
            if ($hint) {
                $hints = InvCeLocation::where("parent", "LIKE", "%" . $hint . "%")
                    ->orderBy("parent")
                    ->limit(100)
                    ->get()
                    ->pluck("parent")
        ...
        if ($property == "loc_bin") {
            ...
            if ($hint) {
                $hints = InvCeLocation::where("bin", "LIKE", "%" . $hint . "%")
                    ->orderBy("bin")
                    ->limit(100)
                    ->get()
                    ->pluck("bin")
...
<div id="content" class="py-6 max-w-8xl mx-auto sm:px-6 lg:px-8 text-neutral-800 dark:text-neutral-200">
    <div class="static lg:sticky top-0 z-10 py-6">
        <div
            class="flex flex-col lg:flex-row w-full bg-white dark:bg-neutral-800 divide-x-0 divide-y lg:divide-x lg:divide-y-0 divide-neutral-200 dark:divide-neutral-700 shadow sm:rounded-lg lg:rounded-full py-0 lg:py-2"
        >
            <div x-data="{ is_linked: @entangle("is_linked").live }" class="flex gap-x-2 items-center px-8 py-2 lg:px-4 lg:py-0">
                ...
                <div x-show="is_linked" class="w-full md:w-32">
                    <x-text-input-t
                        ...
                        id="inv-q"
                        name="inv-q"
                        class="h-9 py-1 placeholder-neutral-400 dark:placeholder-neutral-600"
                        type="search"
                        list="qwords"
                        placeholder="{{ __('Cari...') }}"
                        autofocus
                    ...
                    <x-dropdown align="right" width="60">
                        ...
                        <x-slot name="content">
                            ...
                            <x-dropdown-link href="#" wire:click.prevent="resetQuery">
                                <i class="w-4 icon-rotate-cw me-2"></i>
                                {{ __("Reset") }}
                            </x-dropdown-link>
                            <hr class="border-neutral-300 dark:border-neutral-600" />
                        </x-slot>
    ...
    <div class="h-auto sm:h-12">
        <div class="flex items-center flex-col gap-y-6 sm:flex-row justify-between w-full h-full px-8">
            ...
            <div class="grow flex flex-col sm:flex-row gap-3 items-center justify-center sm:justify-end">
                ...
                <div class="btn-group">
                    <x-radio-button wire:model.live="view" value="list" name="view" id="view-list"><i class="icon-align-justify text-center m-auto"></i></x-radio-button>
                    <x-radio-button wire:model.live="view" value="content" name="view" id="view-content"><i class="icon-layout-list text-center m-auto"></i></x-radio-button>
                    <x-radio-button wire:model.live="view" value="grid" name="view" id="view-grid"><i class="icon-layout-grid text-center m-auto"></i></x-radio-button>
                </div>
            </div>
        </div>
    </div>
        ...
        @if (! $inv_ce_stocks->count())
            ...
            @else
                <div wire:key="no-area" class="py-20">
                    <div class="text-center text-neutral-300 dark:text-neutral-700 text-5xl mb-3">
                        <i class="icon-house relative"><i class="icon-circle-help absolute bottom-0 -right-1 text-lg text-neutral-500 dark:text-neutral-400"></i></i>
                    </div>
                                        ...
                                        <div class="rounded-sm overflow-hidden relative flex w-12 h-4 bg-neutral-200 dark:bg-neutral-700">
                                            <div class="m-auto">
                                                <svg
                                                    ...
                                                    class="block w-4 h-4 fill-current text-neutral-800 dark:text-neutral-200 opacity-25"
