...
new #[Layout("layouts.app")] class extends Component {
    ...
    public function with(): array
    {
        ...
        $machines->each(function ($machine) {
            $machine->latest_metric_obj = $machine
                ->ins_ctc_metrics()
                ->latest("created_at")
            ...
            <div class="bg-white dark:bg-neutral-800 shadow sm:rounded-lg overflow-hidden">
                <div class="flex gap-x-3 items-center px-8 py-4 w-full bg-caldy-200 dark:bg-caldy-700 bg-opacity-20 dark:bg-opacity-10">
                    ...
                    <div class="text-sm text-neutral-500">
                        {{ $machine->latest_metric_obj ? Carbon\Carbon::parse($machine->latest_metric_obj->created_at)->diffForHumans() : __("Belum ada batch terkini") }}
                    </div>
                </div>
                <div>
                    @if ($machine->latest_metric_obj)
                        @php
                            $metric = $machine->latest_metric_obj;
                        @endphp
