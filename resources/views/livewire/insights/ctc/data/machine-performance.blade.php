...
new #[Layout('layouts.app')] class extends Component {
    ...
    private function calculateMachinePerformance($metrics)
    {
        ...
        $this->machinePerformance = $metrics->groupBy('ins_ctc_machine_id')
            ->map(function ($machineMetrics) use ($totalHours) {
                ...
                $overallPerformance = ($qualityScore * 0.3) + ($utilization * 0.25) + 
                                     ($availability * 0.25) + ($consistencyScore * 0.2);
    ...
    private function calculateAvailabilityData($metrics)
    {
        ...
        $this->availabilityData = $metrics->groupBy(function ($metric) {
            return Carbon::parse($metric->created_at)->format('Y-m-d');
        })->map(function ($dayMetrics, $date) {
            $machineActivity = $dayMetrics->groupBy('ins_ctc_machine_id')
    ...
    {
        $this->qualityTrends = $metrics->groupBy(function ($metric) {
            return Carbon::parse($metric->created_at)->format('Y-m-d');
        })->map(function ($dayMetrics, $date) {
            return $dayMetrics->groupBy('ins_ctc_machine_id')
    ...
    {
        $this->productivityTrends = $metrics->groupBy(function ($metric) {
            return Carbon::parse($metric->created_at)->format('Y-m-d');
        })->map(function ($dayMetrics, $date) {
            return $dayMetrics->groupBy('ins_ctc_machine_id')
                ->map(function ($machineMetrics) use ($date) {
    ...
    private function calculateAvailability($machine, $totalHours): float
    {
        ...
        $recentMetrics = $machine->ins_ctc_metrics()
        ...
        $activeDays = $recentMetrics->groupBy(function ($metric) {
            return Carbon::parse($metric->created_at)->format('Y-m-d');
        })->count();
    ...
    {
        if ($metrics->count() < 2) return 100;
        
        // Calculate coefficient of variation for MAE (lower = more consistent)
        $maeValues = $metrics->pluck('t_mae')->filter()->toArray();
        if (empty($maeValues)) return 100;
            ...
            'datasets' => [
                [
                    ...
                    'backgroundColor' => array_map(function($machine) {
                        if ($machine['overall_performance'] >= 80) return '#10B981';
                        if ($machine['overall_performance'] >= 60) return '#F59E0B';
                        return '#EF4444';
                    }, $this->performanceRanking),
                    'borderWidth' => 1,
                ]
    ...
    {
        if (empty($this->qualityTrends)) return;

        // Group by machine for multiple lines
        $machineGroups = collect($this->qualityTrends)->groupBy('machine_line');
        $dates = collect($this->qualityTrends)->pluck('date')->unique()->sort()->values();
        
        $datasets = $machineGroups->map(function ($machineData, $line) use ($dates) {
    ...
    {
        if (empty($this->productivityTrends)) return;

        $machineGroups = collect($this->productivityTrends)->groupBy('machine_line');
        $dates = collect($this->productivityTrends)->pluck('date')->unique()->sort()->values();
    ...
    {
        if (empty($this->machinePerformance)) return;
    ...
    {
        if (empty($this->machinePerformance) || count($this->machinePerformance) < 2) return;

        // Take top 3 performers for comparison
        $topPerformers = array_slice($this->performanceRanking, 0, 3);
        
        $datasets = array_map(function($machine, $index) {
        ...
        }, $topPerformers, array_keys($topPerformers));
                    ...
                    <tbody class="bg-white dark:bg-neutral-800 divide-y divide-neutral-200 dark:divide-neutral-700">
                        @foreach($performanceRanking as $machine)
                            <tr>
                                ...
                                <td class="px-4 py-4 whitespace-nowrap text-sm text-neutral-500 dark:text-neutral-300">
                                    <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium {{ $machine['overall_performance'] > 80 ? 'bg-green-100 text-green-800' : ($machine['overall_performance'] > 60 ? 'bg-yellow-100 text-yellow-800' : 'bg-red-100 text-red-800') }}">
                                        {{ $machine['overall_performance'] }}%
                                    </span>
                                </td>
                                <td class="px-4 py-4 whitespace-nowrap text-sm text-neutral-500 dark:text-neutral-300">
                                    <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium {{ $machine['performance_category'] === 'High' ? 'bg-green-100 text-green-800' : ($machine['performance_category'] === 'Medium' ? 'bg-yellow-100 text-yellow-800' : 'bg-red-100 text-red-800') }}">
