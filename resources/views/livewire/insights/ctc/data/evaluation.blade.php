...
new #[Layout('layouts.app')] class extends Component {
    ...
    private function calculateEvaluationStats($metrics)
    {
        ...
        $avgGood = $metrics->filter(function ($metric) {
            return abs($metric->t_balance) <= 1; // seimbang
        })->count();

        $maeGood = $metrics->filter(function ($metric) {
            return $metric->t_mae <= 1.0; // sesuai standar
        })->count();

        $ssdGood = $metrics->filter(function ($metric) {
            return $metric->t_ssd <= 1.0; // konsisten
        })->count();

        $correctionGood = $metrics->filter(function ($metric) {
            return $metric->correction_uptime > 50; // auto
        })->count();

        // Calculate target achievement (all evaluations good)
        $targetAchievement = $metrics->filter(function ($metric) {
            return abs($metric->t_balance) <= 1 && 
                   $metric->t_mae <= 1.0 && 
                   $metric->t_ssd <= 1.0 && 
                   $metric->correction_uptime > 50;
        })->count();
    ...
    {
        $hourlyProduction = $metrics->groupBy(function ($metric) {
    ...
    {
        $dailyProduction = $metrics->groupBy(function ($metric) {
            return Carbon::parse($metric->created_at)->format('Y-m-d');
        })->map->count();
        
        if ($dailyProduction->count() <= 1) return 100;
    ...
    private function generateEvaluationDistributionCharts()
    {
        ...
        $avgData = [
            ...
            'datasets' => [
                [
                    'data' => [
                        $this->evaluationStats['avg_good_count'] ?? 0,
                        ($this->evaluationStats['total_batches'] ?? 0) - ($this->evaluationStats['avg_good_count'] ?? 0)
                    ],
        ...
        $maeData = [
            ...
            'datasets' => [
                [
                    'data' => [
                        $this->evaluationStats['mae_good_count'] ?? 0,
                        ($this->evaluationStats['total_batches'] ?? 0) - ($this->evaluationStats['mae_good_count'] ?? 0)
                    ],
                    'backgroundColor' => ['#10B981', '#EF4444'],
                ]
            ]
        ...
        $ssdData = [
            ...
            'datasets' => [
                [
                    'data' => [
                        $this->evaluationStats['ssd_good_count'] ?? 0,
                        ($this->evaluationStats['total_batches'] ?? 0) - ($this->evaluationStats['ssd_good_count'] ?? 0)
                    ],
                    'backgroundColor' => ['#10B981', '#EF4444'],
                ]
            ]
                    ...
                    <tbody class="bg-white dark:bg-neutral-800 divide-y divide-neutral-200 dark:divide-neutral-700">
                        @foreach($shiftStats as $shift)
                            <tr>
                                ...
                                <td class="px-4 py-4 whitespace-nowrap text-sm text-neutral-500 dark:text-neutral-300">
                                    <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium {{ $shift['overall_performance'] > 80 ? 'bg-green-100 text-green-800' : ($shift['overall_performance'] > 60 ? 'bg-yellow-100 text-yellow-800' : 'bg-red-100 text-red-800') }}">
