...
new #[Layout("layouts.app")] class extends Component {
    ...
    {
        $this->effectivenessData = $metrics
            ->map(function ($metric) {
                return [
    ...
    private function calculateCorrections($metrics, $side): int
    {
        ...
        foreach ($metrics as $metric) {
            if ($metric->data && is_array($metric->data)) {
                foreach ($metric->data as $point) {
                    if ($side === "left" && ($point[2] ?? 0) > 0) {
                        $total++;
                    }
                    if ($side === "right" && ($point[3] ?? 0) > 0) {
                        $total++;
                    }
                }
            }
    ...
    private function calculateCorrectionEffectiveness($metrics): float
    {
        ...
        if ($highCorrectionBatches->count() === 0) {
            return 0;
        }
    ...
    private function calculateQualityImprovement($metrics): float
    {
        ...
        if ($autoBatches->count() === 0 || $manualBatches->count() === 0) {
            return 0;
        }
    ...
    private function generateAutoVsManualChart()
    {
        $autoData = [
            ...
            "datasets" => [
                [
                    "data" => [$this->correctionStats["auto_batches"], $this->correctionStats["manual_batches"]],
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        <div class="bg-white dark:bg-neutral-800 shadow sm:rounded-lg p-6">
            ...
            <div class="text-2xl font-bold text-green-600">
                {{ $correctionStats["auto_percentage"] ?? 0 }}
        ...
        <div class="bg-white dark:bg-neutral-800 shadow sm:rounded-lg p-6">
            ...
            <div class="text-2xl font-bold">
                {{ $correctionStats["avg_correction_uptime"] ?? 0 }}
                <span class="text-sm font-normal">%</span>
            </div>
        </div>
        <div class="bg-white dark:bg-neutral-800 shadow sm:rounded-lg p-6">
            ...
            <div class="text-2xl font-bold">
                {{ $correctionStats["avg_correction_rate"] ?? 0 }}
                <span class="text-sm font-normal">%</span>
            </div>
        </div>
        <div class="bg-white dark:bg-neutral-800 shadow sm:rounded-lg p-6">
            ...
            <div class="text-2xl font-bold text-blue-600">
                {{ $correctionStats["correction_effectiveness"] ?? 0 }}
                <span class="text-sm font-normal">%</span>
            </div>
        </div>
    </div>
            ...
            <table class="min-w-full divide-y divide-neutral-200 dark:divide-neutral-700">
                ...
                <tbody class="bg-white dark:bg-neutral-800 divide-y divide-neutral-200 dark:divide-neutral-700">
                    @foreach ($shiftStats as $shift)
                        <tr>
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-neutral-900 dark:text-neutral-100">{{ __("Shift") }} {{ $shift["shift"] }}</td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-neutral-500 dark:text-neutral-300">
                                {{ $shift["batch_count"] }}
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-neutral-500 dark:text-neutral-300">
                                <span
                                    class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium {{ $shift["auto_percentage"] > 70 ? "bg-green-100 text-green-800" : ($shift["auto_percentage"] > 40 ? "bg-yellow-100 text-yellow-800" : "bg-red-100 text-red-800") }}"
                                >
                                    {{ $shift["auto_percentage"] }}%
                                </span>
                                <div class="text-xs text-neutral-400">{{ $shift["auto_count"] }}/{{ $shift["batch_count"] }}</div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-neutral-500 dark:text-neutral-300">{{ $shift["avg_cu"] }}%</td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-neutral-500 dark:text-neutral-300">{{ $shift["avg_cr"] }}%</td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-neutral-500 dark:text-neutral-300">
                                <span class="{{ $shift["avg_mae"] <= 1.0 ? "text-green-600" : "text-red-600" }}">{{ $shift["avg_mae"] }} mm</span>
                            </td>
                        </tr>
                    @endforeach
                </tbody>
