...
new #[Layout("layouts.app")] class extends Component {
    ...
    public function with(): array
    {
        ...
        $clumps = DB::table("ins_rtc_metrics")
            ...
            // Assign Shift 1, 2, 3
            ->selectRaw(
                'CASE WHEN HOUR(MIN(ins_rtc_metrics.dt_client)) BETWEEN 6 AND 13 THEN "1" WHEN HOUR(MIN(ins_rtc_metrics.dt_client)) BETWEEN 14 AND 21 THEN "2" ELSE "3" END AS shift',
            )
...
<div class="overflow-auto w-full">
    <div class="flex justify-between items-center mb-6 px-5 py-1">
        <h1 class="text-2xl text-neutral-900 dark:text-neutral-100">{{ __("Ringkasan Gilingan") }}</h1>
        <div class="flex gap-x-1">
            <x-secondary-button type="button" wire:click="$refresh">
                <i class="icon-rotate-cw" wire:loading.remove></i>
                <i class="icon-loader-circle icon-spin" wire:loading></i>
            </x-secondary-button>
            <x-secondary-button type="button" x-data="" x-on:click.prevent="$dispatch('open-modal', 'clumps-info')"><i class="icon-circle-help"></i></x-secondary-button>
        </div>
    </div>
    <x-modal name="clumps-info">
        <div class="p-6">
            ...
            <p class="mt-3 text-sm text-neutral-600 dark:text-neutral-400">
                <span class="font-bold">{{ __("SD:") . " " . "Standard Deviation." . " " }}</span>
                {{ __("Digunakan untuk mengukur kekonsistenan ketebalan. Semakin kecil semakin konsisten.") }}
            </p>
            <p class="mt-3 text-sm text-neutral-600 dark:text-neutral-400">
                <span class="font-bold">{{ __("MAE:") . " " . "Mean Average Error." . " " }}</span>
            ...
            <div class="bg-white dark:bg-neutral-800 shadow sm:rounded-lg table">
                <table class="table table-sm text-sm table-truncate text-neutral-600 dark:text-neutral-400">
                    ...
                    @foreach ($clumps as $clump)
                        <tr
                            wire:key="clump-tr-{{ $clump->id . $loop->index }}"
                            tabindex="0"
                            x-on:click="
                                $dispatch('open-modal', 'clump')
                                $dispatch('clump-load', { id: '{{ $clump->id }}' })
                            "
                        >
                            <td>{{ $clump->id }}</td>
                            <td>{{ $clump->line }}</td>
