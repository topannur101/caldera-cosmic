    ...
    private function renderCharts()
    {
        ...
        $this->js("
            (function() {
                ...
                if (dailyCtx) {
                    window.dailyTrendsChart = new Chart(dailyCtx, " . json_encode($dailyTrendsOptions) . ");
                }
                ...
                if (comparisonCtx) {
                    window.comparisonChart = new Chart(comparisonCtx, " . json_encode($comparisonOptions) . ");
                }
            })();
        ");
    }
...
<div>
            ...
            <div class="grow flex justify-center gap-x-2 items-center">
                <div wire:loading.class.remove="hidden" class="hidden">
                    <x-progress-bar :$progress>                       
                        ...
                        progress < 98 ? '{{ __('Menghitung tren...') }}' : 
                        '{{ __('Merender grafik...') }}'
                        "></span>
                    </x-progress-bar>
                </div>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        ...
        <div class="bg-white dark:bg-neutral-800 shadow sm:rounded-lg p-6">
            ...
            <div class="text-2xl font-bold {{ ($trendMetrics['volatility_temperature'] ?? 0) > 10 ? 'text-red-500' : (($trendMetrics['volatility_temperature'] ?? 0) > 5 ? 'text-yellow-500' : 'text-green-500') }}">
        ...
        <div class="bg-white dark:bg-neutral-800 shadow sm:rounded-lg p-6">
            ...
            <div class="text-2xl font-bold {{ ($trendMetrics['volatility_humidity'] ?? 0) > 10 ? 'text-red-500' : (($trendMetrics['volatility_humidity'] ?? 0) > 5 ? 'text-yellow-500' : 'text-green-500') }}">
