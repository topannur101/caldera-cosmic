    ...
    private function renderCharts($records)
    {
        if ($records->isEmpty()) {
            $this->js("
                ...
                if (container) {
                    container.innerHTML = '<div class=\"flex items-center justify-center h-full text-neutral-500\">" . __('Data tidak tersedia') . "</div>';
                }
            ");
            return;
        }
        ...
        $this->js("
            (function() {
                ...
                if (ctx) {
                    window.climateOverviewChart = new Chart(ctx, " . json_encode($chartOptions) . ");
                }
            })();
        ");
    }
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        ...
        <div class="bg-white dark:bg-neutral-800 shadow sm:rounded-lg p-6">
            ...
            <div class="space-y-1">
                ...
                <div class="flex justify-between text-sm">
                    ...
                    <span class="font-medium">{{ ( $periodStats['temperature']['avg'] ?? 0 ) . ' | ' . ($periodStats['temperature']['max'] ?? 0) - ($periodStats['temperature']['min'] ?? 0) }}°C</span>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white dark:bg-neutral-800 shadow sm:rounded-lg p-6">
            ...
            <div class="space-y-1">
                ...
                <div class="flex justify-between text-sm">
                    <span>{{ __('Rerata') . ' | ' . __('Rentang') }}:</span>
                    <span class="font-medium">{{ ( $periodStats['humidity']['avg'] ?? 0 ) . ' | ' . ($periodStats['humidity']['max'] ?? 0) - ($periodStats['humidity']['min'] ?? 0) }}%</span>
                </div>
            </div>
        </div>
    </div>
