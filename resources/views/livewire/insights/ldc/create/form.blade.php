...
new class extends Component {
    ...
    public function save()
    {
        if (! Auth::user()) {
            ...
            $styles = Cache::get("styles", collect([["name" => $group->style, "updated_at" => now()]]));
            $styles = Caldera::manageCollection($styles, $group->style);
            Cache::put("styles", $styles);

            $lines = Cache::get("lines", collect([["name" => $group->line, "updated_at" => now()]]));
            $lines = Caldera::manageCollection($lines, $group->line);
            Cache::put("lines", $lines);

            $materials = Cache::get("materials", collect([["name" => $group->material, "updated_at" => now()]]));
    ...
    x-data="{
        ...
        initWebSocket() {
            ...
            this.websocket.onopen = () => {
                ...
                console.log(
                    'Terhubung dengan ldc-worker, websocket.readyState: ' +
            ...
            this.websocket.onclose = () => {
                ...
                console.log(
                    'Terputus dengan ldc-worker, websocket.readyState: ' +
                        this.websocket.readyState,
                )
        ...
        get diff() {
            ...
            return area_vn > 0 && area_ab > 0
                ? ((area_vn - area_ab) / area_vn) * 100
                : 0
        },
        get defect() {
            let area_vn = parseFloat(this.area_vn)
            let area_qt = parseFloat(this.area_qt)
            return area_vn > 0 && area_qt > 0
                ? ((area_vn - area_qt) / area_vn) * 100
                : 0
        },
        get area_qt_eval() {
            try {
                let result = eval(
                    this.area_qt_string.replace(/[^\d\.\+\-\*\/\(\)]/g, ''),
                )
        ...
        setCursorToEnd() {
            ...
            this.$refs.hidecode.setSelectionRange(
                this.code.length,
                this.code.length,
            )
        },
...
>
    <form id="ldc-index-form-element" wire:submit="save">
        <div class="grid grid-cols-1 gap-6">
            <div class="flex justify-between text-xs uppercase">
                ...
                <div class="bg-neutral-200 dark:bg-neutral-900 rounded-full px-3 py-1 font-bold" :class="connectionStatusClass">
                    <i class="icon-circlemr-2"></i>
                    <span x-text="connectionMessage">{{ __("ldc-worker terputus") }}</span>
                </div>
            </div>
            <div class="grid grid-cols-3 gap-3">
                ...
                <div>
                    <div class="flex">
                        ...
                        <div class="text-neutral-500 text-xs">
                            <span class="uppercase">{{ __("Selisih") . ": " }}</span>
                            <span x-text="diff.toFixed(1) + '%'"></span>
                        </div>
                    </div>
                ...
                <div>
                    <div class="flex">
                        ...
                        <div class="text-neutral-500 text-xs pr-3">|</div>
                        <div class="text-neutral-500 text-xs">
                            <span class="uppercase">{{ __("Defect") . ": " }}</span>
                            <span x-text="defect.toFixed(1) + '%'"></span>
                        </div>
                    </div>
                    <x-text-input-suffix
                        suffix="SF"
                        ...
                        step=".01"
                        autocomplete="off"
                        x-on:keydown="if ($event.key === '+' || $event.key === '-') { $dispatch('open-spotlight', 'calculate-qt'); console.log(area_qt); area_qt_string = area_qt + $event.key }"
                    />
                </div>
            </div>
            <div class="grid grid-cols-3 gap-3">
                ...
                <div>
                    ...
                    <div class="flex w-full justify-between items-center text-neutral-500 px-3 mt-2 text-xs">
                        @foreach ($ins_ldc_machines as $ins_ldc_machine)
                            <x-text-button x-on:click="code = '{{ $ins_ldc_machine->code }}'; $nextTick(() => setCursorToEnd())" type="button">
                                {{ $ins_ldc_machine->code }}
                            </x-text-button>
                        @endforeach
                    </div>
                </div>
        ...
        <div class="flex justify-between items-end">
            <div class="flex gap-3">
                <div>
                    <label for="hide-shift" class="block px-3 mb-2 uppercase text-xs text-neutral-500">{{ __("Shift") }}</label>
                ...
                <div>
                    <label for="hide-material" class="block px-3 mb-2 uppercase text-xs text-neutral-500">{{ __("Material") }}</label>
            ...
            <div class="flex gap-x-6">
                <x-text-button
                    type="button"
                    class="uppercase text-xs text-red-500 {{ $is_editing ? '' : 'hidden' }}"
                    wire:click="delete"
    ...
    <x-spotlight name="calculate-qt" focusable maxWidth="sm">
        <div class="w-full flex flex-col gap-y-10 pb-10">
            ...
            <div x-text="area_qt_eval" class="text-center font-bold text-7xl">0.00</div>
            <x-text-input-line
                type="text"
                x-model="area_qt_string"
                x-on:keyup.enter="area_qt = area_qt_eval; window.dispatchEvent(escKey); $refs.hidecode.focus()"
            ></x-text-input-line>
        </div>
    </x-spotlight>
