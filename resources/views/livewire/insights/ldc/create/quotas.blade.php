...
new class extends Component {
    ...
    public function deleteQuota()
    {
        ...
        if ($quota) {
            ...
            InsLdcHide::where("ins_ldc_quota_id", $quota->id)->update(["ins_ldc_quota_id" => null]);
...
<div class="py-6">
    <x-slide-over name="quotas">
        <div class="p-6 overflow-auto relative h-full">
            ...
            >
                <div
                    class="relative inline-grid items-center justify-center w-full h-10 grid-cols-2 p-1 text-neutral-500 bg-neutral-200 dark:bg-neutral-900 rounded-full select-none"
                >
                    <button
                        data-tab="manage"
                    ...
                    <div
                        class="absolute left-0 h-full p-1 duration-300 ease-out transition-transform"
            ...
            <div class="flex-1 overflow-y-auto pt-4" x-data="{ tabSelected: @entangle("active_tab") }">
                <div class="h-full">
                    {{-- Manage Tab --}}
                    <div x-show="tabSelected === 'manage'" x-cloak class="p-1">
                        @if ($current_view === "machines")
                            ...
                            <div class="grid grid-cols-2 gap-2">
                                @for ($i = 1; $i <= 20; $i++)
                                    <div
                                        wire:click="showMachine({{ $i }})"
                                        class="border border-neutral-200 dark:border-neutral-700 rounded px-4 py-2 cursor-pointer bg-caldy-400 dark:bg-caldy-700 bg-opacity-0 dark:bg-opacity-0 hover:bg-opacity-10 dark:hover:bg-opacity-10"
                                    >
                        ...
                        @if ($current_view === "machine")
                            ...
                            <div>
                                ...
                                <div class="flex items-center gap-x-2 mb-4 font-medium uppercase text-sm text-neutral-500">
                                    <x-secondary-button type="button" wire:click="backToMachines" class="hover:text-neutral-700 dark:hover:text-neutral-300">
                                ...
                                <div class="space-y-3">
                                    @if (count($machine_quotas) > 0)
                                        @foreach ($machine_quotas as $quota)
                                            <div
                                                wire:click="showQuota({{ $quota["id"] }})"
                                                class="border border-neutral-200 dark:border-neutral-700 rounded p-4 cursor-pointer bg-caldy-400 dark:bg-caldy-700 bg-opacity-0 dark:bg-opacity-0 hover:bg-opacity-10 dark:hover:bg-opacity-10"
                                            >
                                                <div>
                        ...
                        @if ($current_view === "quota")
                            ...
                            <div>
                                {{-- Nav --}}
                                <div class="flex items-center gap-x-2 mb-4 font-medium uppercase text-sm text-neutral-500">
                                    <x-secondary-button type="button" wire:click="backToMachine" class="hover:text-neutral-700 dark:hover:text-neutral-300">
                                        <i class="icon-arrow-left"></i>
                                    </x-secondary-button>
                                    <div class="ml-2">{{ "MC " . $selected_machine }}</div>
                                                    ...
                                                    @foreach ($quota_hides as $hide)
                                                        <tr class="border-b border-neutral-100 dark:border-neutral-800">
                                                            ...
                                                            <td class="p-1 text-right">
                                                                <x-text-button
                                                                    type="button"
                                                                    wire:click="removeHideFromQuota({{ $hide['id'] }})"
                    ...
                    <div x-show="tabSelected === 'pin'" x-cloak class="p-1">
                        ...
                        <div class="grid grid-cols-2 gap-2">
                            @for ($i = 1; $i <= 20; $i++)
                                <div
                                    wire:click="togglePin({{ $i }})"
                                    class="border rounded px-4 py-2 cursor-pointer bg-caldy-400 dark:bg-caldy-700 bg-opacity-0 dark:bg-opacity-0 hover:bg-opacity-10 dark:hover:bg-opacity-10 {{ in_array($i, $cmachines) ? "border-caldy-500" : "border-neutral-200 dark:border-neutral-700" }}"
                                >
    ...
    <div class="text-center">
        <x-text-button class="mb-2" x-on:click="$dispatch('open-slide-over', 'quotas')">
            <div class="uppercase text-xs text-neutral-500">
                {{ __("Kuota") }}
                <i class="icon-settings-2 ml-1"></i>
            </div>
        </x-text-button>
    </div>
    <div class="whitespace-nowrap text-nowrap text-xs w-full">
        <input
            ...
            id="quota-0"
            value="0"
            wire:model="selected_quota_for_form"
            @click="$dispatch('set-form-quota', { quota_id: 0 })"
            class="peer hidden"
        />
        <label
            for="quota-0"
            class="block h-full cursor-pointer px-4 py-3 bg-caldy-400 dark:bg-caldy-700 bg-opacity-0 dark:bg-opacity-0 hover:bg-opacity-10 dark:hover:bg-opacity-10 peer-checked:bg-opacity-100 dark:peer-checked:bg-opacity-100 peer-checked:text-white"
        >
            ...
            <div wire:key="pinned-machine-{{ $machine_data["machine"] }}">
                @if ($machine_data["quota_id"])
                    <input
                        type="radio"
                        name="selected_quota"
                        id="quota-{{ $machine_data["quota_id"] }}"
                        value="{{ $machine_data["quota_id"] }}"
                        wire:model="selected_quota_for_form"
                        @click="$dispatch('set-form-quota', { quota_id: {{ $machine_data["quota_id"] }} })"
                        class="peer hidden"
                    />
                    <label
                        for="quota-{{ $machine_data["quota_id"] }}"
                        class="block h-full cursor-pointer px-4 py-3 bg-caldy-400 dark:bg-caldy-700 bg-opacity-0 dark:bg-opacity-0 peer-checked:bg-opacity-100 dark:peer-checked:bg-opacity-100 peer-checked:text-white hover:bg-opacity-10 dark:hover:bg-opacity-10"
                    >
                        <div class="flex justify-between items-center gap-x-3">
                            ...
                            <div class="w-20 bg-neutral-200 rounded-full h-1 dark:bg-neutral-700">
                                <div
                                    class="bg-white h-1 rounded-full peer-checked:bg-white opacity-60 peer-checked:opacity-100"
                                    style="width: {{ $machine_data["progress"] }}%"
                                ></div>
                            </div>
                        </div>
                    </label>
