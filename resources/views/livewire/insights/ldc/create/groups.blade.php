...
<div wire:key="index-groups-container" class="flex items-stretch whitespace-nowrap text-nowrap text-sm min-h-20">
    <div class="p-1">
        <x-text-button type="button" x-data="" class="p-3 h-full text-lg" x-on:click.prevent="$dispatch('open-modal', 'group-set')"><i class="icon-plus"></i></x-text-button>
    </div>
        ...
        @foreach ($groups as $group)
            <div wire:key="{{ $loop->iteration . $group["id"] }}">
                <input
                    type="radio"
                    name="sgid"
                    id="sgid-{{ $loop->iteration }}"
                    value="{{ $group["id"] }}"
                    wire:model="sgid"
                    @click="$dispatch('set-form-group', { group_id: '{{ $group["id"] }}', material: '{{ $group["material"] }}' })"
                    :checked="{{ $group["id"] == $sgid ? "true" : "false" }}"
                    class="peer hidden"
                />
                <label
                    for="sgid-{{ $loop->iteration }}"
                ...
                >
                    <div class="flex items-center justify-between text-lg">
                        <div>
                            {{ $group["line"] }}
                            <span class="text-xs uppercase ml-1 mr-2">{{ Carbon::parse($group["workdate"])->format("d M") }}</span>
                        </div>
                    </div>
