...
new #[Layout("layouts.app")] class extends Component {
    ...
    public function download()
    {
        ...
        $columns = [__("Diperbarui"), __("Kode"), __("VN"), __("AB"), __("QT"), __("G"), __("S"), __("WO"), __("Style"), __("Line"), __("Material"), __("NIK"), __("Nama")];
...
<div>
        ...
        <div x-show="!show" class="flex flex-col lg:flex-row gap-3 w-full bg-white dark:bg-neutral-800 shadow sm:rounded-lg p-4">
            <div>
                ...
                <div class="grid gap-3">
                    ...
                    <div class="px-3">
                        <x-checkbox id="hides_is_workdate" wire:model.live="is_workdate" value="is_workdate">
            ...
            <div class="grid gap-3">
                <div class="flex gap-3">
                    <div class="w-full lg:w-32">
                        <label for="hides-machine" class="block px-3 mb-2 uppercase text-xs text-neutral-500">{{ __("Mesin") }}</label>
                        <x-text-input id="hides-machine" wire:model.live="style" type="number" />
                    </div>
                    <div class="w-full lg:w-32">
                        <label for="hides-line" class="block px-3 mb-2 uppercase text-xs text-neutral-500">{{ __("Line") }}</label>
                        <x-text-input id="hides-line" wire:model.live="line" type="text" />
                    </div>
                    <div class="w-full lg:w-32">
                        <label for="hides-style" class="block px-3 mb-2 uppercase text-xs text-neutral-500">{{ __("Style") }}</label>
            ...
            <div class="flex flex-row lg:flex-col">
                <div class="mb-1">
                    <label for="hides-sort" class="block px-3 mb-2 uppercase text-xs text-neutral-500">{{ __("Urut") }}</label>
            ...
            <div>
                <div class="max-w-xl mx-auto px-6 py-8">
                    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                        <div>
                            <label for="quota-machine" class="block px-3 mb-2 uppercase text-xs text-neutral-500">{{ __("Mesin") }}</label>
                            <x-text-input id="quota-machine" type="number" step="1" />
                        </div>
                        <div>
                            <label for="quota-value" class="block px-3 mb-2 uppercase text-xs text-neutral-500">{{ __("Jatah") }}</label>
    ...
    <div wire:key="modals">
        ...
        <x-modal name="hide-edit" maxWidth="sm">
            <livewire:insights.ldc.data.hide-edit />
        </x-modal>
    </div>
    @if (! $hides->count())
        @if (! $start_at || ! $end_at)
        ...
        <div wire:key="raw-hides" class="overflow-auto p-0 sm:p-1">
            <div class="bg-white dark:bg-neutral-800 shadow sm:rounded-lg table">
                <table class="table table-sm text-sm table-truncate text-neutral-600 dark:text-neutral-400">
                    ...
                    @foreach ($hides as $hide)
                        <tr
                            wire:key="hide-tr-{{ $hide->id . $loop->index }}"
                            tabindex="0"
                            x-on:click="
                                $dispatch('open-modal', 'hide-edit')
                                $dispatch('hide-edit', { id: '{{ $hide->id }}' })
                            "
                        >
                            ...
                            <td>
                                <div class="flex items-center">
                                    <div class="relative bg-neutral-200 rounded-full h-1.5 dark:bg-neutral-700 w-16">
                                        <div class="bg-caldy-600 h-1.5 rounded-full dark:bg-caldy-500 transition-all duration-200" style="width: 50%"></div>
                                    </div>
