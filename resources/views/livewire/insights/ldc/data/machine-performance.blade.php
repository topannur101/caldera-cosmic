...
new class extends Component {
    ...
    private function getHidesQuery()
    {
        ...
        $query = InsLdcHide::join("ins_ldc_groups", "ins_ldc_hides.ins_ldc_group_id", "=", "ins_ldc_groups.id")->select(
            "ins_ldc_hides.*",
    ...
    private function calculateMachineStats()
    {
        ...
        foreach ($machineStats as $machine => &$stats) {
            $stats["avg_variance"] = count($stats["variances"]) > 0 ? round(array_sum($stats["variances"]) / count($stats["variances"]), 2) : 0;
            $stats["avg_qt_percentage"] = count($stats["qt_percentages"]) > 0 ? round(array_sum($stats["qt_percentages"]) / count($stats["qt_percentages"]), 2) : 0;
            $stats["avg_vn"] = $stats["total_hides"] > 0 ? round($stats["total_vn"] / $stats["total_hides"], 2) : 0;
            $stats["avg_ab"] = $stats["total_hides"] > 0 ? round($stats["total_ab"] / $stats["total_hides"], 2) : 0;
        }
        ...
        <div class="flex flex-col lg:flex-row gap-3 w-full bg-white dark:bg-neutral-800 shadow sm:rounded-lg p-4">
            <div>
                ...
                <div class="grid gap-3">
                    ...
                    <div class="px-3">
                        <x-checkbox id="machine_is_workdate" wire:model.live="is_workdate" value="is_workdate">
                        ...
                        <span
                            x-text="
                                progress < 25
                                    ? '{{ __("Mengambil data...") }}'
                                    : progress < 70
                                      ? '{{ __("Menghitung statistik mesin...") }}'
                                      : progress < 98
                                        ? '{{ __("Menganalisis tren akurasi...") }}'
                                        : '{{ __("Merender grafik...") }}'
                            "
                        ></span>
                    </x-progress-bar>
