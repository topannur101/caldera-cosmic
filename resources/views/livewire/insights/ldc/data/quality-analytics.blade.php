...
new class extends Component {
    ...
    private function renderCharts(array $hides)
    {
        ...
        $this->js(
            ...
            (function() {
                ...
                if (differenceCtx) {
                    window.differenceChart = new Chart(differenceCtx, " .
                json_encode($differenceOptions) .
                ");
                }
                ...
                json_encode($distributionOptions) .
                ");
                }
            })();
        ",
    ...
    public function showMaterialDetails(string $material)
    {
        ...
        if ($this->materialDetails["total_vn"] > 0) {
            $this->materialDetails["defect_rate"] = (($this->materialDetails["total_vn"] - $this->materialDetails["total_qt"]) / $this->materialDetails["total_vn"]) * 100;
        }
...
<div>
        ...
        <div class="flex flex-col lg:flex-row gap-3 w-full bg-white dark:bg-neutral-800 shadow sm:rounded-lg p-4">
            ...
            <div>
                ...
                <div class="grid gap-3">
                    ...
                    <div class="px-3">
                        <x-checkbox id="quality_analytics_is_workdate" wire:model.live="is_workdate" value="is_workdate">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        ...
        <div class="bg-white dark:bg-neutral-800 shadow sm:rounded-lg p-6">
            ...
            <div
                class="text-2xl font-bold {{ ($qualityMetrics["overall_defect_rate"] ?? 0) > 15 ? "text-red-500" : (($qualityMetrics["overall_defect_rate"] ?? 0) > 10 ? "text-yellow-500" : "text-green-500") }}"
            >
        ...
        <div class="bg-white dark:bg-neutral-800 shadow sm:rounded-lg p-6">
            ...
            <div
                class="text-2xl font-bold {{ abs($qualityMetrics["overall_difference_rate"] ?? 0) > 5 ? "text-red-500" : (abs($qualityMetrics["overall_difference_rate"] ?? 0) > 2 ? "text-yellow-500" : "text-green-500") }}"
            >
        ...
        <div class="bg-white dark:bg-neutral-800 shadow sm:rounded-lg p-6">
            ...
            @if ($qualityMetrics["best_material"] ?? false)
                <div class="text-xs text-neutral-500">{{ round($qualityMetrics["material_stats"][$qualityMetrics["best_material"]]["defect_rate"], 2) }}% defect</div>
            @endif
        </div>
    </div>
                    ...
                    @if (isset($qualityMetrics["material_stats"]))
                        @foreach ($qualityMetrics["material_stats"] as $material => $stats)
                            <tr class="border-b border-neutral-100 dark:border-neutral-700">
                                ...
                                <td class="px-4 py-3">
                                    <x-text-button type="button" wire:click="showMaterialDetails('{{ $material }}')" class="text-xs">{{ __("Detail") }}</x-text-button>
                                </td>
                            </tr>
                        @endforeach
                    @else
                ...
                @if (isset($qualityMetrics["grade_stats"]))
                    @foreach ($qualityMetrics["grade_stats"] as $grade => $stats)
                        <div class="text-center p-4 border border-neutral-200 dark:border-neutral-700 rounded-lg">
                            ...
                            <div class="text-sm text-neutral-600 dark:text-neutral-400">{{ round($stats["avg_qt_percent"], 1) }}% QT</div>
                        </div>
                    @endforeach
                @endif
            </div>
    ...
    <x-modal name="material-details" maxWidth="md">
        <div class="p-6">
            <div class="flex justify-between items-start">
                <h2 class="text-lg font-medium text-neutral-900 dark:text-neutral-100">{{ __("Detail Material") }}: {{ $materialDetails["name"] ?? "" }}</h2>
                ...
                <div class="mt-6 space-y-4">
                    <div class="grid grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <div
                                class="text-xl font-semibold {{ $materialDetails["defect_rate"] > 15 ? "text-red-500" : ($materialDetails["defect_rate"] > 10 ? "text-yellow-500" : "text-green-500") }}"
                            >
