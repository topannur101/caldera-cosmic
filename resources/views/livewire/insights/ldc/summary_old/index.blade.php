    ...
    <div class="flex flex-col gap-x-2 md:gap-x-4 sm:flex-row min-w-0">
        <div>
            <div class="w-full sm:w-44 md:w-64 px-3 sm:px-0 mb-5">
                ...
                <div class="bg-white dark:bg-neutral-800 shadow rounded-lg py-5 px-4 {{ $is_date ? "" : "hidden" }}">
                    ...
                    <div class="mt-5">
                        <x-radio id="is_workdate_false" wire:model.live="is_workdate" name="is_workdate" :checked="!$is_workdate" value="">{{ __("Tanggal catat") }}</x-radio>
                        <x-radio id="is_workdate_true" wire:model.live="is_workdate" name="is_workdate" :checked="$is_workdate" value="true">{{ __("Tanggal WO") }}</x-radio>
                    </div>
                </div>
                <div class="mt-4 bg-white dark:bg-neutral-800 shadow rounded-lg py-5 px-4 {{ $is_filter ? "" : "hidden" }}">
                    ...
                    <div>
                        <x-text-input wire:model.live="fquery" class="mt-4" type="search" placeholder="{{ __('Kata kunci') }}" name="fquery" />
                    </div>
                </div>
        ...
        @switch($view)
            @case("hides")
                <livewire:insights.ldc.data.hides :$start_at :$end_at :$is_workdate :$fquery :$ftype />
