...
new #[Layout("layouts.app")] class extends Component {
    ...
    public function loadBatch(
        ...
        string $code_alt,
        string $omv_eval,
        string $omv_eval_human,
        string $rdc_eval,
        string $rdc_eval_human,
        int $rdc_tests_count,
    ) {
        $this->id = $id;
        $this->code = $code ?: "-";
            ...
            <div>
                <ol class="relative border-s border-neutral-200 dark:border-neutral-700 mt-2">
                    <li class="mb-6 ms-4">
                        ...
                        <x-pill
                            class="uppercase"
                            color="{{ $omv_eval === 'on_time' ? 'green' : ($omv_eval === 'on_time_manual' ? 'yellow' : ($omv_eval === 'too_late' || $omv_eval === 'too_soon' ? 'red' : 'neutral')) }}"
                        >
                            {{ $omv_eval_human }}
                        </x-pill>
                    </li>
