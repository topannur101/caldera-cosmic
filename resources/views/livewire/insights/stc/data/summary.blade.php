...
new class extends Component {
    ...
    {
        $machines = InsStcMachine::whereNot("code", "like", "test%")
            ->orderBy("line")
            ->get()
            ->toArray();
        ...
        foreach ($machines as $key => $machine) {
            ...
            $machines[$key]["upper"]["d_sum"]["hb_diff"] = array_map(
                function ($hb, $tv) {
                    return $hb ? $hb - $tv : 0;
                },
                $upper_d_sum_hb_values,
                $tvs,
            );
            ...
            $machines[$key]["lower"]["d_sum"]["hb_diff"] = array_map(
                function ($hb, $tv) {
                    return $hb ? $hb - $tv : 0;
                },
                $lower_d_sum_hb_values,
                $tvs,
            );
...
<div wire:poll.60s>
        ...
        <x-modal name="different-zoning-help">
            <div class="p-6">
                ...
                <div class="grid gap-y-6 mt-6 text-sm text-neutral-600 dark:text-neutral-400">
                    ...
                    <div>
                        <span class="font-bold">{{ __("Pembagian zona standar") . ": " }}</span>
                        {{ __("Menggunakan semua titik data dan ditempatkan pada pembagian zona standar pada panjang data yang tetap sebesar 54 menit.") }}
                    </div>
                    <div>
                        <span class="font-bold">{{ __("Pembagian zona adaptif") . ": " }}</span>
                        {{ __("Mengabaikan titik data yang dianggap temperatur ruangan (di bawah 40°C) agar SV prediksi akurat sehingga pembagian zona akan tergantung dengan panjang data.") }}
                    </div>
                </div>
        ...
        <div class="px-8 py-4 text-sm">
            @switch($present_mode)
                ...
                @case("standard")
                    ...
                    @break
            @endswitch
            <x-text-button type="button" class="ml-2" x-data="" x-on:click="$dispatch('open-modal', 'different-zoning-help')"><i class="far icon-circle-help"></i></x-text-button>
        </div>
    ...
    <div class="bg-white dark:bg-neutral-800 shadow sm:rounded-lg h-80 p-4 mb-8" id="recents-chart-container" wire:key="recents-chart-container" wire:ignore></div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-4">
        @foreach ($machines as $machine)
            <div class="bg-white dark:bg-neutral-800 shadow sm:rounded-lg text-sm overflow-hidden">
                <div class="flex">
                    <div
                        class="flex items-center border-r border-neutral-100 dark:border-neutral-700 p-4 font-mono text-2xl {{ substr($machine["code"], 0, 3) == "OLD" ? "bg-caldy-500 bg-opacity-20" : "" }}"
                    >
                        <div>
                            <div>
                            ...
                            @if (strpos($machine["ip_address"], "127.") !== 0)
                                <i class="icon-badge-check text-caldy-500" title="{{ __("Kontrol otomatis") }}"></i>
                            @endif
                        </div>
                    </div>
                    <div class="grow">
                        <div class="flex items-center border-b border-neutral-100 dark:border-neutral-700">
                            <div class="px-2">△</div>
                            <div class="grow grid grid-cols-1 py-3">
                                <div>
                                    <div class="grid grid-cols-8 text-center px-3 gap-x-3 mb-1 text-xs uppercase font-normal leading-none text-neutral-500">
                                        <div>1</div>
                                        <div>2</div>
                                        <div>3</div>
                                        <div>4</div>
                                    ...
                                    <div class="grid grid-cols-8 text-center px-3 gap-x-3">
                                        <div class="rounded {{ $machine["upper"]["d_sum"]["hb_eval"][0] ? "bg-red-500 bg-opacity-10 text-red-500" : "" }}">
                                            {{ $machine["upper"]["d_sum"]["hb_values"][0] ?? "-" }}
                                        </div>
                                        <div class="rounded {{ $machine["upper"]["d_sum"]["hb_eval"][1] ? "bg-red-500 bg-opacity-10 text-red-500" : "" }}">
                                            {{ $machine["upper"]["d_sum"]["hb_values"][1] ?? "-" }}
                                        </div>
                                        <div class="rounded {{ $machine["upper"]["d_sum"]["hb_eval"][2] ? "bg-red-500 bg-opacity-10 text-red-500" : "" }}">
                                            {{ $machine["upper"]["d_sum"]["hb_values"][2] ?? "-" }}
                                        </div>
                                        <div class="rounded {{ $machine["upper"]["d_sum"]["hb_eval"][3] ? "bg-red-500 bg-opacity-10 text-red-500" : "" }}">
                                            {{ $machine["upper"]["d_sum"]["hb_values"][3] ?? "-" }}
                                        </div>
                                        <div class="rounded {{ $machine["upper"]["d_sum"]["hb_eval"][4] ? "bg-red-500 bg-opacity-10 text-red-500" : "" }}">
                                            {{ $machine["upper"]["d_sum"]["hb_values"][4] ?? "-" }}
                                        </div>
                                        <div class="rounded {{ $machine["upper"]["d_sum"]["hb_eval"][5] ? "bg-red-500 bg-opacity-10 text-red-500" : "" }}">
                                            {{ $machine["upper"]["d_sum"]["hb_values"][5] ?? "-" }}
                                        </div>
                                        <div class="rounded {{ $machine["upper"]["d_sum"]["hb_eval"][6] ? "bg-red-500 bg-opacity-10 text-red-500" : "" }}">
                                            {{ $machine["upper"]["d_sum"]["hb_values"][6] ?? "-" }}
                                        </div>
                                        <div class="rounded {{ $machine["upper"]["d_sum"]["hb_eval"][7] ? "bg-red-500 bg-opacity-10 text-red-500" : "" }}">
                                            {{ $machine["upper"]["d_sum"]["hb_values"][7] ?? "-" }}
                                        </div>
                                    </div>
                                </div>
                            </div>
                            <div class="flex px-2 flex-col">
                                ...
                                <x-link
                                    href="{{ route('insights.stc.data.index', [ 'view' => 'history', 'mode' => 'recents', 'line' => $machine['line'], 'position' => 'upper' ])}}"
                                    class="px-3 py-2"
                                    wire:navigate
                                >
                        ...
                        <div class="flex items-center">
                            ...
                            <div class="grow grid grid-cols-1 py-3">
                                <div>
                                    ...
                                    <div class="grid grid-cols-8 text-center px-3 gap-x-3">
                                        <div class="rounded {{ $machine["lower"]["d_sum"]["hb_eval"][0] ? "bg-red-500 bg-opacity-10 text-red-500" : "" }}">
                                            {{ $machine["lower"]["d_sum"]["hb_values"][0] ?? "-" }}
                                        </div>
                                        <div class="rounded {{ $machine["lower"]["d_sum"]["hb_eval"][1] ? "bg-red-500 bg-opacity-10 text-red-500" : "" }}">
                                            {{ $machine["lower"]["d_sum"]["hb_values"][1] ?? "-" }}
                                        </div>
                                        <div class="rounded {{ $machine["lower"]["d_sum"]["hb_eval"][2] ? "bg-red-500 bg-opacity-10 text-red-500" : "" }}">
                                            {{ $machine["lower"]["d_sum"]["hb_values"][2] ?? "-" }}
                                        </div>
                                        <div class="rounded {{ $machine["lower"]["d_sum"]["hb_eval"][3] ? "bg-red-500 bg-opacity-10 text-red-500" : "" }}">
                                            {{ $machine["lower"]["d_sum"]["hb_values"][3] ?? "-" }}
                                        </div>
                                        <div class="rounded {{ $machine["lower"]["d_sum"]["hb_eval"][4] ? "bg-red-500 bg-opacity-10 text-red-500" : "" }}">
                                            {{ $machine["lower"]["d_sum"]["hb_values"][4] ?? "-" }}
                                        </div>
                                        <div class="rounded {{ $machine["lower"]["d_sum"]["hb_eval"][5] ? "bg-red-500 bg-opacity-10 text-red-500" : "" }}">
                                            {{ $machine["lower"]["d_sum"]["hb_values"][5] ?? "-" }}
                                        </div>
                                        <div class="rounded {{ $machine["lower"]["d_sum"]["hb_eval"][6] ? "bg-red-500 bg-opacity-10 text-red-500" : "" }}">
                                            {{ $machine["lower"]["d_sum"]["hb_values"][6] ?? "-" }}
                                        </div>
                                        <div class="rounded {{ $machine["lower"]["d_sum"]["hb_eval"][7] ? "bg-red-500 bg-opacity-10 text-red-500" : "" }}">
                                            {{ $machine["lower"]["d_sum"]["hb_values"][7] ?? "-" }}
                                        </div>
                                    </div>
                                </div>
                            </div>
                            <div class="flex px-2 flex-col">
                                ...
                                <x-link
                                    href="{{ route('insights.stc.data.index', [ 'view' => 'history', 'mode' => 'recents', 'line' => $machine['line'], 'position' => 'lower' ])}}"
                                    class="px-3 py-2"
                                    wire:navigate
                                >
                                    <i class="icon-history"></i>
