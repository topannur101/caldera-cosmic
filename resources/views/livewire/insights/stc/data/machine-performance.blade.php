...
new class extends Component {
    ...
    public function updated()
    {
        ...
        $machines = InsStcMachine::with([
            "ins_stc_d_sums" => function ($query) use ($start, $end) {
    ...
    private function calculateMachinePerformance($machines, $targets)
    {
        ...
        foreach ($machines as $machine) {
            ...
            $stats[$machine->line]["overall"] = $this->calculateOverallStats($stats[$machine->line]["upper"], $stats[$machine->line]["lower"]);
        }
                ...
                <div class="space-y-6 text-sm text-neutral-600 dark:text-neutral-400">
                    <div>
                        ...
                        <div class="bg-neutral-50 dark:bg-neutral-800 p-3 rounded text-xs font-mono">
                            {{ __("Rumus: 100 - (AvgDeviation × 10)") }}
                            <br />
                            {{ __("Deviasi rendah = skor tinggi = lebih akurat") }}
                            <br />
                    ...
                    <div>
                        ...
                        <div class="bg-neutral-50 dark:bg-neutral-800 p-3 rounded text-xs font-mono">
                            {{ __("Rumus: 100 - ((StdDev / Mean) × 100)") }}
                            <br />
                            {{ __("CV rendah = skor tinggi = lebih konsisten") }}
                            <br />
                    ...
                    <div>
                        ...
                        <div class="bg-neutral-50 dark:bg-neutral-800 p-3 rounded text-xs font-mono">
                            {{ __("Rumus: (Jumlah penyetelan otomatis / Total pengukuran) × 100") }}
                            <br />
                            {{ __("Tinggi = lebih banyak otomasi = lebih baik") }}
                            <br />
                    ...
                    <div class="border-t border-neutral-200 dark:border-neutral-700 pt-4">
                        ...
                        <div class="bg-neutral-50 dark:bg-neutral-800 p-3 rounded text-xs font-mono">
                            {{ __("Skor Peringkat = (Akurasi × 50%) + (Konsistensi × 50%)") }}
                            <br />
                    ...
                    <tbody>
                        @foreach ($performanceRanking as $index => $machine)
                            <tr class="border-b border-neutral-100 dark:border-neutral-700">
                                <td class="px-4 py-3">
                                    @if ($index === 0)
                                        <span class="inline-flex items-center px-2 py-1 text-xs font-medium bg-yellow-100 text-yellow-800 rounded-full">🥇 #{{ $index + 1 }}</span>
                                    @elseif ($index === 1)
                                        <span class="inline-flex items-center px-2 py-1 text-xs font-medium bg-gray-100 text-gray-800 rounded-full">🥈 #{{ $index + 1 }}</span>
                                    @elseif ($index === 2)
                                        <span class="inline-flex items-center px-2 py-1 text-xs font-medium bg-orange-100 text-orange-800 rounded-full">🥉 #{{ $index + 1 }}</span>
                                    @else
                                ...
                                <td class="px-4 py-3">
                                    <div class="flex items-center">
                                        <span
                                            class="text-sm font-bold {{ $machine["overall"]["ranking_score"] >= 80 ? "text-green-600" : ($machine["overall"]["ranking_score"] >= 60 ? "text-yellow-600" : "text-red-600") }}"
                                        >
                                ...
                                <td class="px-4 py-3">
                                    <div class="flex items-center">
                                        <div class="mr-2 w-16 bg-neutral-200 rounded-full h-2">
                                            <div
                                                class="h-2 rounded-full {{ $machine["overall"]["accuracy"] >= 80 ? "bg-green-500" : ($machine["overall"]["accuracy"] >= 60 ? "bg-yellow-500" : "bg-red-500") }}"
                                                style="width: {{ $machine["overall"]["accuracy"] }}%"
                                            ></div>
                                        </div>
                                        <span
                                            class="text-sm {{ $machine["overall"]["accuracy"] >= 80 ? "text-green-600" : ($machine["overall"]["accuracy"] >= 60 ? "text-yellow-600" : "text-red-600") }}"
                                        >
                                ...
                                <td class="px-4 py-3">
                                    <div class="flex items-center">
                                        <div class="mr-2 w-16 bg-neutral-200 rounded-full h-2">
                                            <div
                                                class="h-2 rounded-full {{ $machine["overall"]["consistency"] >= 80 ? "bg-green-500" : ($machine["overall"]["consistency"] >= 60 ? "bg-yellow-500" : "bg-red-500") }}"
                                                style="width: {{ $machine["overall"]["consistency"] }}%"
                                            ></div>
                                        </div>
                                        <span
                                            class="text-sm {{ $machine["overall"]["consistency"] >= 80 ? "text-green-600" : ($machine["overall"]["consistency"] >= 60 ? "text-yellow-600" : "text-red-600") }}"
                                        >
                                ...
                                <td class="px-4 py-3">
                                    <span
                                        class="text-sm {{ $machine["overall"]["automation_rate"] >= 60 ? "text-green-600" : ($machine["overall"]["automation_rate"] >= 30 ? "text-yellow-600" : "text-red-600") }}"
                                    >
