...
new class extends Component {
    ...
    public function update()
    {
        ...
        $query = InsStcDSum::with(["ins_stc_machine", "ins_stc_d_logs"])->whereBetween("created_at", [$start, $end]);
    ...
    private function generateCharts($dSums)
    {
        ...
        $this->js(
            ...
            (function() {
                ...
                "',
                            data: " .
                json_encode(array_column($zoneData, "y")) .
                ",
                            color: '#D64550'
                        },
                        {
                ...
                "',
                            data: " .
                json_encode(array_column($zoneData, "target")) .
                ",
                            color: '#10B981'
                        }
                    ],
                    xaxis: {
                        categories: " .
                json_encode(array_column($zoneData, "x")) .
                "
                    },
                    yaxis: {
                        title: { text: '°C' }
                ...
                json_encode(InsStc::getStandardZoneChartOptions($d_sums_grouped, 100, 100)) .
                ";
    ...
    <div class="p-0 sm:p-1 mb-6">
        <div class="flex flex-col lg:flex-row gap-3 w-full bg-white dark:bg-neutral-800 shadow sm:rounded-lg p-4">
            ...
            <div class="flex gap-3">
                ...
                <div>
                    <label for="analysis-position" class="block px-3 mb-2 uppercase text-xs text-neutral-500">{{ __("Posisi") }}</label>
                    ...
                    <tbody class="divide-y divide-neutral-200 dark:divide-neutral-700">
                        @foreach ($zone_stats as $section => $stats)
                            <tr>
                                ...
                                <td class="px-6 py-4">
                                    <span class="{{ $stats["mean_deviation"] > 5 ? "text-red-500" : "text-green-500" }}">{{ $stats["mean_deviation"] }}°C</span>
                                </td>
                                <td class="px-6 py-4">{{ $stats["std_dev"] }}°C</td>
                                <td class="px-6 py-4">{{ $stats["range"] }}°C</td>
                                <td class="px-6 py-4">
                                    <span class="{{ $stats["compliance_rate"] < 95 ? "text-red-500" : "text-green-500" }}">{{ $stats["compliance_rate"] }}%</span>
                                </td>
                            </tr>
                        @endforeach
                    </tbody>
