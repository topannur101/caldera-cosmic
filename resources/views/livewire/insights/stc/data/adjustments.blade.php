...
new class extends Component {
    ...
    private function getAdjustmentsQuery()
    {
        ...
        return $dSumQuery->unionAll($adjustQuery)->orderBy("created_at", "DESC");
    }
    ...
    public function download()
    {
        ...
        $columns = [__("Waktu"), __("Line"), __("Posisi"), __("Tipe"), __("AT"), __("Delta"), __("SV Awal"), __("SV Akhir"), __("Status"), __("Operator")];
            ...
            <div class="bg-white dark:bg-neutral-800 shadow sm:rounded-lg table">
                <table class="table table-sm text-sm table-truncate text-neutral-600 dark:text-neutral-400">
                    ...
                    @foreach ($adjustments as $adjustment)
                        ...
                        <tr
                            wire:key="adjustment-tr-{{ $adjustment->id . $loop->index }}"
                            tabindex="0"
                            x-on:click="
                                $dispatch('open-modal', 'd_sum-show')
                                $dispatch('d_sum-show', { id: '{{ $targetId }}' })
                            "
                        >
                            ...
                            <td
                                class="{{ $adjustment->record_type === "adjustment" && $adjustment->delta_temp > 0 ? "text-red-600" : ($adjustment->record_type === "adjustment" && $adjustment->delta_temp < 0 ? "text-blue-600" : "") }}"
                            >
