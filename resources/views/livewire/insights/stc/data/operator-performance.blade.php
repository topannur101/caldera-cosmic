...
new class extends Component {
    ...
    private function calculateOperatorPerformance()
    {
        ...
        $query = InsStcDSum::with(["user", "ins_stc_machine"])->whereBetween("created_at", [$start, $end]);
        ...
        foreach ($dSums as $dSum) {
            ...
            if ($dSum->ended_at && $dSum->created_at) {
                $latencyMinutes = Carbon::parse($dSum->ended_at)->diffInMinutes(Carbon::parse($dSum->created_at));
            }
        ...
        foreach ($operatorData as $data) {
            if ($data["total_measurements"] >= $this->min_measurements) {
                ...
                $avgLatency = $data["total_measurements"] > 0 ? $data["total_latency"] / $data["total_measurements"] : 0;
                ...
                $efficiencyScore = round($latencyScore * 0.3 + $complianceMetrics["compliance_score"] * 0.3 + $distributionScore * 0.2 + $sequenceScore * 0.2, 1);
    ...
    private function calculateComplianceMetrics(array $shifts, int $expectedPerShift): array
    {
        ...
        $complianceScore = $totalExpected > 0 ? min(100, round(($totalActual / $totalExpected) * 100)) : 0;
                ...
                if (count($lineMeasurements) >= 2) {
                    ...
                    for ($i = 1; $i < count($lineMeasurements); $i++) {
                        ...
                        if ($prev["sequence"] == "1" && $curr["sequence"] == "2") {
                            $timeDiff = Carbon::parse($prev["created_at"])->diffInHours(Carbon::parse($curr["created_at"]));
        ...
        $this->js(
            ...
            (function() {
                ...
                const efficiencyOptions = {
                    ...
                    data: {
                        labels: " .
                json_encode(array_slice($chartData["labels"], 0, 15)) .
                ",
                        datasets: [
                            {
                                label: '" .
                ...
                "',
                                data: " .
                json_encode(array_slice($chartData["efficiency"], 0, 15)) .
                ",
                    ...
                    options: {
                        ...
                        plugins: {
                            title: {
                                display: true,
                                text: '" .
                __("Skor Efisiensi Operator (Top 15)") .
                "'
                            },
                            legend: {
                                display: false
                ...
                ",
                        datasets: [
                            {
                                label: '" .
                __("Latensi Upload (menit)") .
                "',
                                data: " .
                json_encode(array_slice($chartData["latency"], 0, 15)) .
                ",
                                backgroundColor: function(context) {
                                    const value = context.parsed.y;
    ...
    public function getAlertLevel($stats): string
    {
        ...
        if ($stats["avg_latency_minutes"] > 30) {
            return "critical";
        }
        if ($stats["missing_measurements"] > 10) {
            return "warning";
        }
        if ($stats["efficiency_score"] < 75) {
            return "warning";
        }
        return "good";
    }
    ...
    <div class="p-0 sm:p-1 mb-6">
        <div class="flex flex-col lg:flex-row gap-3 w-full bg-white dark:bg-neutral-800 shadow sm:rounded-lg p-4">
            ...
            <div class="grid grid-cols-2 lg:flex gap-3">
                ...
                <div>
                    <label for="min-measurements" class="block px-3 mb-2 uppercase text-xs text-neutral-500">{{ __("Min. Pengukuran") }}</label>
                ...
                <div>
                    <label for="cycle-gap-hours" class="block px-3 mb-2 uppercase text-xs text-neutral-500">{{ __("Gap Siklus (jam)") }}</label>
            ...
            <table class="min-w-full divide-y divide-neutral-200 dark:divide-neutral-700">
                ...
                <tbody class="bg-white dark:bg-neutral-800 divide-y divide-neutral-200 dark:divide-neutral-700">
                    @forelse ($operator_stats as $index => $stat)
                        <tr
                            class="hover:bg-neutral-50 dark:hover:bg-neutral-700 {{ $this->getAlertLevel($stat) == "critical" ? "bg-red-50 dark:bg-red-900/20" : ($this->getAlertLevel($stat) == "warning" ? "bg-yellow-50 dark:bg-yellow-900/20" : "") }}"
                        >
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-neutral-900 dark:text-neutral-100">
                                <div class="flex items-center">
                                    ...
                                    <span class="text-lg font-bold {{ $index < 3 ? "text-yellow-600" : "text-neutral-600" }}">#{{ $index + 1 }}</span>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-center">
                                <div class="text-sm font-medium {{ $this->getPerformanceClass($stat["compliance_score"]) }}">{{ $stat["compliance_score"] }}%</div>
                                <div class="text-xs text-neutral-500">{{ $stat["actual_measurements"] }}/{{ $stat["expected_measurements"] }}</div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-center text-sm">
                                @if ($stat["missing_measurements"] > 0)
                                    <div class="text-red-600 font-medium">-{{ $stat["missing_measurements"] }} {{ __("kurang") }}</div>
                                @endif

                                @if ($stat["excess_measurements"] > 0)
                                    <div class="text-blue-600">+{{ $stat["excess_measurements"] }} {{ __("lebih") }}</div>
                                @endif
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-center">
                                ...
                                @elseif ($stat["efficiency_score"] >= 75)
                                    <span
                                        class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-lime-100 text-lime-800 dark:bg-lime-800 dark:text-lime-100"
                                    >
                                        Good
                                    </span>
    ...
    @if (count($operator_stats) >= 3)
        <div class="mt-6 grid grid-cols-1 md:grid-cols-3 gap-4">
            @foreach (array_slice($operator_stats, 0, 3) as $index => $stat)
                <div
                    class="bg-white dark:bg-neutral-800 shadow sm:rounded-lg p-4 border-l-4 {{ $index == 0 ? "border-yellow-400" : ($index == 1 ? "border-gray-400" : "border-yellow-600") }}"
                >
                    <div class="flex items-center justify-between mb-3">
                        <h4 class="font-medium text-neutral-900 dark:text-neutral-100">{{ $this->getRankIcon($index) }} {{ $stat["name"] }}</h4>
    ...
    <div class="mt-6 bg-white dark:bg-neutral-800 shadow sm:rounded-lg p-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
                ...
                <dd class="text-sm text-neutral-600 dark:text-neutral-400">
                    <i class="icon-alert-triangle text-red-500 mr-1"></i>
                    {{ __("Critical: Efisiensi <60 atau latensi >30 min") }}
                    <br />
                    <i class="icon-alert-circle text-yellow-500 mr-1"></i>
                    {{ __("Warning: Efisiensi <75 atau missing >10") }}
                </dd>
            </div>
        </div>
