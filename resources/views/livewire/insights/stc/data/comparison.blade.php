...
new class extends Component {
    ...
    {
        $this->machines = InsStcMachine::orderBy("line")
            ->get()
            ->toArray();
    }
...
<div>
    <div wire:key="data-selected-d_sums-container" class="px-8 flex items-center gap-x-6 mb-8">
        ...
        @else
            <div>
                ...
                {{ __("Belum ada hasil ukur yang dipilih") }}
            </div>
        @endif
        <x-secondary-button type="button" x-data="" x-on:click.prevent="$dispatch('open-modal', 'group-set')">{{ __("Sisipkan") }}</x-secondary-button>
    </div>
    <div wire:key="modals">
    ...
    <div wire:key="stc-data-comparison">
        <div wire:key="stc-data-comparison-chart" class="relative bg-white dark:bg-neutral-800 shadow sm:rounded-lg p-4 sm:p-6 overflow-hidden">
            <div id="stc-data-comparison-chart-container" class="h-96" wire:key="stc-data-comparison-chart-container" wire:ignore></div>
