...
new class extends Component {
    ...
    private function calculateTrendStats($trendData): array
    {
        ...
        if ($trendData->count() > 0) {
            ...
            $firstHalf = $chronologicalData
                ->take(ceil($chronologicalData->count() / 2))
                ->pluck("average")
                ->avg();
            $secondHalf = $chronologicalData
                ->skip(floor($chronologicalData->count() / 2))
                ->pluck("average")
                ->avg();
    ...
    private function prepareChartData($trendData): array
    {
        ...
        $colors = ["#D64550", "#36A2EB", "#FFCE56", "#4BC0C0", "#9966FF", "#FF9F40", "#C9CBCF", "#4BC0C0"];
        $colorIndex = 0;
        ...
        <div class="bg-white dark:bg-neutral-800 shadow sm:rounded-lg p-4">
            ...
            <div class="text-lg">
                @switch($trend_stats["temp_trend"] ?? "stable")
                    @case("increasing")
                        <i class="icon-trending-up text-red-500"></i>
                        {{ __("Meningkat") }}

                        @break
                    @case("decreasing")
                        <i class="icon-trending-down text-blue-500"></i>
                        {{ __("Menurun") }}

                        @break
                    @default
                        <i class="icon-minus text-green-500"></i>
                        {{ __("Stabil") }}
                @endswitch
            </div>
        </div>
