                     ...
                     options: {
                        ...
                        plugins: {
                           title: {
                                 display: true,
                                 text: '" .
                __("Klasifikasi Deviasi per Section") .
                "'
                           }
                        },
                        scales: {
                     ...
                     options: {
                        ...
                        plugins: {
                              title: {
                                 display: true,
                                 text: '" .
                __("Klasifikasi Deviasi per Line") .
                "'
                              }
                        },
                        scales: {
                ...
                <div class="space-y-6 text-sm text-neutral-600 dark:text-neutral-400">
                    ...
                    <div>
                        ...
                        <div class="bg-neutral-50 dark:bg-neutral-800 p-3 rounded text-xs font-mono space-y-2">
                            <div>
                                <strong>{{ __("Tingkat Deviasi:") }}</strong>
                                {{ __("% pengukuran dengan deviasi ≥3°C") }}
                            </div>
                            <div>
                                <strong>{{ __("Tingkat Major+:") }}</strong>
                                {{ __("% pengukuran dengan deviasi ≥6°C") }}
                            </div>
                            <div>
                                <strong>{{ __("Tingkat Critical:") }}</strong>
                                {{ __("% pengukuran dengan deviasi >9°C") }}
                            </div>
                            <div>
                                <strong>{{ __("Rata-rata Deviasi:") }}</strong>
                                {{ __("Σ(Nilai Aktual - Target) / Total Pengukuran") }}
                            </div>
                            <div>
                                <strong>{{ __("Standar Deviasi:") }}</strong>
                                {{ __("Mengukur konsistensi/variabilitas deviasi") }}
                            </div>
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <div class="space-y-2 text-xs">
                            <div>
                                <strong>{{ __("Rata-rata Deviasi Rendah + StdDev Rendah:") }}</strong>
                                {{ __("Performa sangat baik, konsisten mendekati target") }}
                            </div>
                            <div>
                                <strong>{{ __("Rata-rata Deviasi Rendah + StdDev Tinggi:") }}</strong>
                                {{ __("Akurat tapi tidak konsisten, perlu stabilisasi") }}
                            </div>
                            <div>
                                <strong>{{ __("Rata-rata Deviasi Tinggi + StdDev Rendah:") }}</strong>
                                {{ __("Konsisten tapi bias sistemik, perlu kalibrasi") }}
                            </div>
                            <div>
                                <strong>{{ __("Rata-rata Deviasi Tinggi + StdDev Tinggi:") }}</strong>
                                {{ __("Performa buruk, perlu investigasi menyeluruh") }}
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="border-t border-neutral-200 dark:border-neutral-700 pt-4">
                        ...
                        <div class="space-y-2 text-xs">
                            <div class="flex items-start">
                                ...
                                <div>
                                    <strong>{{ __("Baik (0-3°C):") }}</strong>
                                    {{ __("Pertahankan kondisi saat ini, monitoring rutin") }}
                                </div>
                            </div>
                            <div class="flex items-start">
                                <div class="w-4 h-4 bg-yellow-500 rounded mr-2 mt-1 flex-shrink-0"></div>
                                <div>
                                    <strong>{{ __("Minor (3-6°C):") }}</strong>
                                    {{ __("Monitoring intensif, evaluasi trend") }}
                                </div>
                            </div>
                            <div class="flex items-start">
                                <div class="w-4 h-4 bg-orange-500 rounded mr-2 mt-1 flex-shrink-0"></div>
                                <div>
                                    <strong>{{ __("Major (6-9°C):") }}</strong>
                                    {{ __("Review setting, periksa sensor, evaluasi proses") }}
                                </div>
                            </div>
                            <div class="flex items-start">
                                <div class="w-4 h-4 bg-red-500 rounded mr-2 mt-1 flex-shrink-0"></div>
                                <div>
                                    <strong>{{ __("Critical (>9°C):") }}</strong>
                                    {{ __("Inspeksi menyeluruh") }}
                                </div>
                            </div>
                        </div>
                    </div>
    ...
    @if (! empty($overallStats))
        <div class="grid grid-cols-1 sm:grid-cols-4 gap-4 mb-8">
            ...
            <div class="bg-white dark:bg-neutral-800 shadow sm:rounded-lg p-6">
                ...
                <div
                    class="text-2xl font-bold {{ $overallStats["deviation_rate"] <= 15 ? "text-green-500" : ($overallStats["deviation_rate"] <= 25 ? "text-yellow-500" : "text-red-500") }}"
                >
            ...
            <div class="bg-white dark:bg-neutral-800 shadow sm:rounded-lg p-6">
                ...
                <div
                    class="text-2xl font-bold {{ $overallStats["major_plus_rate"] <= 8 ? "text-green-500" : ($overallStats["major_plus_rate"] <= 15 ? "text-yellow-500" : "text-red-500") }}"
                >
            ...
            <div class="bg-white dark:bg-neutral-800 shadow sm:rounded-lg p-6">
                ...
                <div
                    class="text-2xl font-bold {{ $overallStats["critical_rate"] <= 3 ? "text-green-500" : ($overallStats["critical_rate"] <= 8 ? "text-yellow-500" : "text-red-500") }}"
                >
                ...
                <tbody>
                    @foreach ($sectionSeverity as $section)
                        <tr class="border-b border-neutral-100 dark:border-neutral-700">
                            ...
                            <td class="px-4 py-3">
                                <span class="{{ $section["severity_class"]["class"] }}">{{ $section["avg_deviation"] > 0 ? "+" : "" }}{{ $section["avg_deviation"] }}°C</span>
                            </td>
                            <td>
                ...
                <tbody>
                    @foreach ($lineSeverity as $line)
                        <tr class="border-b border-neutral-100 dark:border-neutral-700">
                            ...
                            <td class="px-4 py-3">
                                <span class="{{ $line["severity_class"]["class"] }}">{{ $line["avg_deviation"] > 0 ? "+" : "" }}{{ $line["avg_deviation"] }}°C</span>
                            </td>
                            <td class="px-4 py-3">
                            ...
                            <td class="px-4 py-3">
                                <div class="flex items-center">
                                    <div class="mr-2 w-16 bg-neutral-200 rounded-full h-2">
                                        <div
                                            class="h-2 rounded-full {{ $line["deviation_rate"] <= 15 ? "bg-green-500" : ($line["deviation_rate"] <= 25 ? "bg-yellow-500" : "bg-red-500") }}"
                                            style="width: {{ min($line["deviation_rate"], 100) }}%"
                                        ></div>
                                    </div>
                                    <span
                                        class="text-sm {{ $line["deviation_rate"] <= 15 ? "text-green-600" : ($line["deviation_rate"] <= 25 ? "text-yellow-600" : "text-red-600") }}"
                                    >
                                        {{ $line["deviation_rate"] }}%
                                    </span>
                                </div>
