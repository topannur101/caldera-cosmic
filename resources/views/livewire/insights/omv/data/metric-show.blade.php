    ...
    <div class="p-6">
        ...
        <div wire:key="capture-selected-none" class="{{ $capture_n ? "hidden" : "" }}">
            <div wire:key="amps-exists" class="{{ $amps_exists ? "" : "hidden" }}">
                <div wire:key="modal-chart-container" wire:ignore class="h-96 mt-8 overflow-hidden" id="modal-chart-container"></div>
            </div>
            <div wire:key="amps-none" class="{{ $amps_exists ? "hidden" : "" }} py-6 rounded-lg my-6">
                <div class="text-center text-neutral-300 dark:text-neutral-700 text-5xl mb-3">
                    <i class="icon-zap relative"><i class="icon-circle-help absolute bottom-0 -right-1 text-lg text-neutral-500 dark:text-neutral-400"></i></i>
                </div>
        ...
        <div wire:key="capture-selected-any" class="{{ $capture_n ? "" : "hidden" }}">
            <div class="w-full h-96 mt-8 overflow-hidden rounded-lg flex items-center justify-center relative group">
                @if (count($captures) && $capture_n > 0)
                    <img src="{{ asset("storage/omv-captures/" . $captures[$capture_n - 1]["file_name"]) }}" class="object-cover w-full h-full" alt="Capture {{ $capture_n }}" />
                    ...
                    @if (count($captures) > 1)
                        ...
                        @if ($capture_n > 1)
                            <button
                                ...
                                class="absolute left-2 top-1/2 -translate-y-1/2 w-8 h-8 flex justify-center items-center bg-black/50 rounded-full text-white opacity-0 group-hover:opacity-100 transition-opacity hover:bg-black/70"
                            >
                        ...
                        @if ($capture_n < count($captures))
                            <button
                                ...
                                class="absolute right-2 top-1/2 -translate-y-1/2 w-8 h-8 flex justify-center items-center bg-black/50 rounded-full text-white opacity-0 group-hover:opacity-100 transition-opacity hover:bg-black/70"
                            >
                    ...
                    <div class="relative w-full h-8">
                        ...
                        @foreach ($captures as $capture_i => $capture)
                            <button
                                ...
                                class="absolute top-1/2 -translate-y-1/2 -translate-x-1/2 w-2 h-2 rounded-full transition-colors {{ $capture_i == $capture_n - 1 ? "bg-caldy-500 z-10 opacity-100" : "bg-neutral-300 hover:bg-neutral-400 dark:bg-neutral-600 dark:hover:bg-neutral-700 opacity-50" }}"
            ...
            <div class="flex flex-col grow">
                ...
                <dd>
                    ...
                    <div>
                        ...
                        <x-pill class="uppercase" color="{{ $rdc_eval === 'queue' ? 'yellow' : ($rdc_eval === 'pass' ? 'green' : ($rdc_eval === 'fail' ? 'red' : 'neutral')) }}">
                            {{ $rdc_eval_human }}
                        </x-pill>
                    </div>
                </dd>
                ...
                <div class="flex flex-col md:flex-row gap-6">
                    ...
                    <div class="grow">
                        ...
                        <div>
                            ...
                            <x-pill
                                class="uppercase"
                                color="{{ $eval === 'on_time' ? 'green' : ($eval === 'on_time_manual' ? 'yellow' : ($eval === 'too_late' || $eval === 'too_soon' ? 'red' : 'neutral')) }}"
                            >
                                {{ $eval_human }}
                            </x-pill>
                        </div>
