...
new #[Layout("layouts.app")] class extends Component {
    ...
    private function getMetricsQuery()
    {
        ...
        $query = InsOmvMetric::with(["ins_omv_recipe", "user_1", "user_2", "ins_rubber_batch"])->whereBetween("start_at", [$start, $end]);
    ...
    <div class="p-0 sm:p-1 mb-6">
        <div class="flex flex-col lg:flex-row gap-3 w-full bg-white dark:bg-neutral-800 shadow sm:rounded-lg p-4">
            ...
            <div class="flex gap-3">
                ...
                <div class="w-full lg:w-28">
                    <label for="batch-mcs" class="block px-3 mb-2 uppercase text-xs text-neutral-500">MCS</label>
            ...
            <div class="bg-white dark:bg-neutral-800 shadow sm:rounded-lg table">
                <table class="table table-sm text-sm table-truncate text-neutral-600 dark:text-neutral-400">
                    <tr class="uppercase text-xs">
                        ...
                        <th>{{ __("Durasi") }}</th>
                        <th>kWh</th>
                        <th><i class="icon-images"></i></th>
                        <th>{{ __("Mulai") }}</th>
                    </tr>
                    @foreach ($metrics as $metric)
                        <tr
                            wire:key="metric-tr-{{ $metric->id . $loop->index }}"
                            tabindex="0"
                            x-on:click="
                                $dispatch('open-modal', 'batch-show')
                                $dispatch('batch-show', { omv_metric_id: '{{ $metric->id }}', view: 'omv' })
                            "
                        >
                            <td>{{ $metric->id }}</td>
                            ...
                            <td>{{ $metric->line }}</td>
                            <td>{{ $metric->team }}</td>
                            <td title="{{ __("Operator 2") . ": " . ($metric->user_2->emp_id ?? "") . " - " . ($metric->user_2->name ?? "") }}">
                                {{ ($metric->user_1->emp_id ?? "") . " - " . ($metric->user_1->name ?? "") }}
                            </td>
                            <td>
                                <x-pill
