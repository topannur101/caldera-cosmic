...
new #[Layout("layouts.app")] class extends Component {
    ...
    public function update()
    {
        ...
        $this->duration_avg = round(
            $metrics->get()->average(function ($metric) {
        ...
        $data = $metrics
            ...
            ->map(function ($lineMetrics) {
                return [
                    "too_soon" => $lineMetrics->where("eval", "too_soon")->sum(function ($metric) {
                        return round(Carbon::parse($metric->start_at)->diffInMinutes(Carbon::parse($metric->end_at)) / 60, 1);
                    }),
                    "on_time" => $lineMetrics->where("eval", "on_time")->sum(function ($metric) {
                        return round(Carbon::parse($metric->start_at)->diffInMinutes(Carbon::parse($metric->end_at)) / 60, 1);
                    }),
                    "on_time_manual" => $lineMetrics->where("eval", "on_time_manual")->sum(function ($metric) {
                        return round(Carbon::parse($metric->start_at)->diffInMinutes(Carbon::parse($metric->end_at)) / 60, 1);
                    }),
                    "too_late" => $lineMetrics->where("eval", "too_late")->sum(function ($metric) {
                        return round(Carbon::parse($metric->start_at)->diffInMinutes(Carbon::parse($metric->end_at)) / 60, 1);
                    }),
                ];
            })
...
<div>
    ...
    <div wire:key="omv-summary-running-time" class="grid grid-cols-1 sm:grid-cols-3 lg:grid-cols-4 gap-x-3">
        ...
        <div class="sm:col-span-2 lg:col-span-3">
            <h1 class="uppercase text-sm text-neutral-500 mb-4 px-8">{{ __("Waktu jalan") }}</h1>
            <div
                wire:key="omv-summary-running-time-chart"
                class="h-[32rem] bg-white dark:bg-neutral-800 shadow sm:rounded-lg p-4 sm:p-6 overflow-hidden"
                id="omv-summary-running-time-chart-container"
                wire:key="omv-summary-running-time-chart-container"
                wire:ignore
            ></div>
        </div>
    </div>
