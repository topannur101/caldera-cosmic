...
new #[Layout("layouts.app")] class extends Component {
    ...
    public function with(): array
    {
        ...
        foreach ($metrics as $metric) {
            ...
            $usersToProcess = [];
            if ($user1Id !== null) {
                $usersToProcess[] = $user1Id;
            }
            if ($user2Id !== null && $user2Id !== $user1Id) {
                $usersToProcess[] = $user2Id;
            }
...
<div>
    ...
    <div wire:key="omv-summary-worker-perf" class="grid grid-cols-1 lg:grid-cols-2 gap-x-3 gap-y-8">
        <div>
            <h1 class="uppercase text-sm text-neutral-500 mb-4 px-8">{{ __("Tepat waktu tertinggi") }}</h1>
            @if ($highOnTimeUsers)
                <div class="grid grid-cols-1 md:grid-cols-3 gap-x-3 gap-y-6 mb-6 mx-8">
                        ...
                        @if (isset($highOnTimeUsers[1]))
                            <div class="relative flex flex-col items-center text-neutral-600 dark:text-neutral-400">
                                <div class="relative w-16 h-16 mb-2">
                                    <div class="absolute -top-1 -left-1 text-sm bg-gray-400 text-black rounded-full w-6 h-6 flex items-center justify-center font-bold">2</div>
                                ...
                                <div class="mt-2">
                                    {{ $highOnTimeUsers[1]["on_time_ratio"] . "%" . " (" . $highOnTimeUsers[1]["total_count_on_time"] . "/" . $highOnTimeUsers[1]["total_count"] . ")" }}
                                </div>
                            </div>
                        @endif
                    </div>
                    <div class="order-1 md:order-2 mt-6 md:mt-0">
                        @if (isset($highOnTimeUsers[0]))
                            <div class="relative flex flex-col items-center text-neutral-600 dark:text-neutral-400">
                                <div class="relative w-16 h-16 mb-2">
                                    <div class="absolute -top-1 -left-1 text-sm bg-yellow-400 text-black rounded-full w-6 h-6 flex items-center justify-center font-bold">1</div>
                                    <div class="w-16 h-16 shadow bg-neutral-200 dark:bg-neutral-700 rounded-full overflow-hidden">
                                ...
                                <div class="mt-2">
                                    {{ $highOnTimeUsers[0]["on_time_ratio"] . "%" . " (" . $highOnTimeUsers[0]["total_count_on_time"] . "/" . $highOnTimeUsers[0]["total_count"] . ")" }}
                                </div>
                            </div>
                        @endif
                    </div>
                    <div class="mt-0 md:mt-8 order-3">
                        @if (isset($highOnTimeUsers[2]))
                            <div class="relative flex flex-col items-center text-neutral-600 dark:text-neutral-400">
                                <div class="relative w-16 h-16 mb-2">
                                    <div class="absolute -top-1 -left-1 text-sm bg-orange-300 text-black rounded-full w-6 h-6 flex items-center justify-center font-bold">3</div>
                                    <div class="w-16 h-16 shadow bg-neutral-200 dark:bg-neutral-700 rounded-full overflow-hidden">
                                        @if ($highOnTimeUsers[1]["photo"])
                                ...
                                <div class="mt-2">
                                    {{ $highOnTimeUsers[2]["on_time_ratio"] . "%" . " (" . $highOnTimeUsers[2]["total_count_on_time"] . "/" . $highOnTimeUsers[2]["total_count"] . ")" }}
                                </div>
                            </div>
                        @endif
                    </div>
                            ...
                            @foreach ($highOnTimeUsers as $highOnTimeUser)
                                @if ($loop->iteration > 3)
                                    <tr>
                                            ...
                                            <div class="flex gap-x-2">
                                                <div class="w-4 h-4 inline-block bg-neutral-200 dark:bg-neutral-700 rounded-full overflow-hidden">
                                                    @if ($highOnTimeUser["photo"] ?? false)
                                                        <img class="w-full h-full object-cover dark:brightness-75" src="{{ "/storage/users/" . $highOnTimeUser["photo"] }}" />
                                                    @else
                                                        <svg
                                                            xmlns="http://www.w3.org/2000/svg"
                                                            class="block fill-current text-neutral-800 dark:text-neutral-200 opacity-25"
                                        ...
                                        <td>
                                            {{ $highOnTimeUser["on_time_ratio"] . "%" . " (" . $highOnTimeUser["total_count_on_time"] . "/" . $highOnTimeUser["total_count"] . ")" }}
                                        </td>
                                    </tr>
                                @endif
                            @endforeach
            ...
            @else
                <div class="py-20">
                    <div class="text-center text-neutral-300 dark:text-neutral-700 text-5xl mb-3">
                        <i class="icon-layout-list relative"><i class="icon-circle-help absolute bottom-0 -right-1 text-lg text-neutral-500 dark:text-neutral-400"></i></i>
                    </div>
                    <div class="text-center text-neutral-400 dark:text-neutral-600">{{ __("Tak ada data") }}</div>
                </div>
            @endif
        </div>
        <div>
            <h1 class="uppercase text-sm text-neutral-500 mb-4 px-8">{{ __("Jumlah batch tertinggi") }}</h1>
            @if ($highBatchUsers)
                <div class="grid grid-cols-1 md:grid-cols-3 gap-x-3 gap-y-6 mb-6 mx-8">
                    <div class="mt-0 md:mt-8 order-2 md:order-1">
                                            ...
                                            <div class="flex gap-x-2">
                                                <div class="w-4 h-4 inline-block bg-neutral-200 dark:bg-neutral-700 rounded-full overflow-hidden">
                                                    @if ($highBatchUser["photo"] ?? false)
                                                        <img class="w-full h-full object-cover dark:brightness-75" src="{{ "/storage/users/" . $highBatchUser["photo"] }}" />
                                                    @else
                                                        <svg
                                                            xmlns="http://www.w3.org/2000/svg"
                                                            class="block fill-current text-neutral-800 dark:text-neutral-200 opacity-25"
