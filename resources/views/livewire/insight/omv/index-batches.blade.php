                        ...
                        <li class="w-full hover:bg-caldy-500 hover:bg-opacity-10">
                            <x-link href="#" x-on:click="$dispatch('open-modal', 'batch-show'); $dispatch('batch-show', { omv_metric_id: '{{ $metric->id }}', view: 'omv'})" 
                                ...
                                <div class="flex flex-wrap gap-1 -mx-2 text-sm">
                                    ...
                                    <x-pill class="inline-block uppercase"
                                    color="{{ $metric->ins_rubber_batch->ins_rdc_test->eval === 'queue' ? 'yellow' : ($metric->ins_rubber_batch->ins_rdc_test->eval === 'pass' ? 'green' : ($metric->ins_rubber_batch->ins_rdc_test->eval === 'fail' ? 'red' : 'neutral')) }}">{{ 'RHEO: ' . $metric->ins_rubber_batch->ins_rdc_test->evalHuman() }}</x-pill>                             
                                    @endif
                                </div>
                            </x-link>
                        </li>
