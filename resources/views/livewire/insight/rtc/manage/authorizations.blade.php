...
new #[Layout('layouts.app')] class extends Component {
   use WithPagination;
    ...
    {
        $q = trim($this->q);
        $auths = InsRtcAuth::join('users', 'ins_rtc_auths.user_id', '=', 'users.id')
            ->select('ins_rtc_auths.*', 'users.name as user_name', 'users.emp_id as user_emp_id', 'users.photo as user_photo')

            ->orderBy('ins_rtc_auths.user_id', 'desc');
...
?>
<x-slot name="title">{{ __('Wewenang') . ' — ' . __('Rubber thickness control') }}</x-slot>
<x-slot name="header">
            ...
            <div>
                <h2 class="font-semibold text-xl text-neutral-800 dark:text-neutral-200 leading-tight">
                    <x-link href="{{ route('insight.rtc.manage.index') }}" class="inline-block py-6" wire:navigate><i
                            class="fa fa-arrow-left"></i></x-link><span class="ml-4">{{ __('Wewenang RTC') }}</span>
                </h2>
            </div>
        </div>
    </header>
