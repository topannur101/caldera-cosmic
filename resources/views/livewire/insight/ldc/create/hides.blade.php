...
new class extends Component {
    ...
    {
        $hides = InsLdcHide::where('updated_at', '>=', Carbon::now()->subDay())
                ...
                <li class="w-full hover:bg-caldy-500 hover:bg-opacity-10">
                    <div class="flex items-center">
                        ...
                        <label for="hs-id-{{ $loop->iteration }}"
                            ...
                            <div>{{ $hide->code }}</div>
                            <div class="text-xs">{{ __('Selisih') . ': ' . ($hide->area_vn > 0 && $hide->area_ab > 0 ? round((($hide->area_vn - $hide->area_ab) / $hide->area_vn) * 100, 1) . ' %' : '?') }}<span class="mx-2">|</span>{{ __('Defect') . ': ' . ($hide->area_vn > 0 && $hide->area_qt > 0 ? round((($hide->area_vn - $hide->area_qt) / $hide->area_vn) * 100, 1) . ' %' : '?') }}
                            </div>
                        </label>
                    </div>
                </li>
