...
new class extends Component
{
   ...
   public function show(string $view = '', int $rdc_test_id = 0)
   {
      ...
      if ($testArray) {
         ...
         $this->test['updated_at'] = Carbon::parse($this->test['updated_at'])->setTimezone('Asia/Jakarta')->format('Y-m-d H:i:s');
...
<div>
   <div class="flex w-full justify-between p-3 border border-neutral-300 dark:border-neutral-700 rounded-full">
      ...
      <div>
         <x-pill class="block uppercase" color="{{ $test['eval'] === 'queue' ? 'yellow' : ($test['eval'] === 'pass' ? 'green' : ($test['eval'] === 'fail' ? 'red' : '')) }}">{{ $test['eval_human'] }}</x-pill> 
      </div>
   </div>
