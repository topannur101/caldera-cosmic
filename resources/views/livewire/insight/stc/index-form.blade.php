...
new class extends Component {
    ...
    public function save()
    {
        if( !Auth::user() ) {
            ...
            $group = InsRdcGroup::firstOrCreate([
                'line'      => $this->line,
            ...
            $this->js('document.getElementById("rdc-index-groups").scrollLeft = 0;');

            $hide = InsRdcHide::updateOrCreate(
                [ 
                    'code' => $this->code 
                ], 
                [
                    'ins_rdc_group_id' => $group->id,
    ...
    public function delete()
    {
        if( !Auth::user() ) {
            ...
            if($this->code) {
                $hide = InsRdcHide::where('code', $this->code);
                if ($hide) {
                    $hide->delete();
...
<div x-data="{ 
    ...
    <form id="rdc-index-form-element" wire:submit="save">
...
@script
<script>
  const form = document.getElementById('rdc-index-form-element');
