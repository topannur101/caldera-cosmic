...
class extends Component {
    ...
    {
        return [
            'batches' => InsRubberBatch::where('stc_eval', 'queue')->orderBy('updated_at')->get()
        ];
    }
    ...
    public function batchQuery()
    {
        ...
        if ($this->code) {
            ...
            $this->js('$dispatch("open-modal", "batch-info"); $dispatch("batch-load", { 
                ...
                color: "' . $batch->color . '", 
                mcs: "' . $batch->mcs . '", 
                stc_eval: "' . $batch->stc_eval . '", 
                stc_eval_human: "' . $batch->stcEvalHuman() . '"
            })');
            $this->reset(['code']);
        } else {
...
<div id="content" class="py-12 max-w-7xl mx-auto sm:px-6 lg:px-8 text-neutral-800 dark:text-neutral-200">
    <div class="flex flex-col gap-y-8 sm:flex-row">
        ...
        <div class="px-8">
            <form wire:submit="batchQuery" class="btn-group">
                <x-text-input class="w-20" wire:model="code" id="stc-code" placeholder="{{ __('Kode') }}"></x-text-input->
    ...
    <div wire:key="batch-info">
        <x-modal name="batch-info">
            <livewire:insight.stc.index-batch-info  />
        </x-modal>
    </div>
    @can('manage', InsStcTest::class)
    <div wire:key="batch-test-create">
        <x-modal name="batch-test-create">
            <livewire:insight.stc.index-batch-test-create  />
        </x-modal>
    </div>
    @endcan
